...
int HeuristicSearcherV35::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV35::TTProbeResult HeuristicSearcherV35::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV35::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void HeuristicSearcherV35::update_best_range(
...
void HeuristicSearcherV35::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
