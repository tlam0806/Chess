...
int HeuristicSearcherV34::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV34::TTProbeResult HeuristicSearcherV34::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV34::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void HeuristicSearcherV34::update_best_range(
...
void HeuristicSearcherV34::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
