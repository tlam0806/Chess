...
void NnueSearcherV37::reward_quiet_cutoff(
    Color side_to_move,
    int depth,
    int ply,
...
void NnueSearcherV37::penalize_failed_quiets(
    Color side_to_move,
    int depth,
    Move prev_move,
    PieceType prev_moved_piece,
    const NnueSearcherV37::ScoredMoveList& failed_quiet_moves
) {
...
void NnueSearcherV37::record_beta_cutoff(
    const ScoredMove& scored_move,
...
bool NnueSearcherV37::should_stop(SearchState& state) const {
...
KingSafetyContext NnueSearcherV37::current_king_safety_context(const Position& pos) const {
    return cached_king_safety_context(pos, pos.side_to_move);
}

int NnueSearcherV37::evaluate_current_position(const Position& pos, const SearchState& state) const {
...
NnueSearcherV37::SearchValue NnueSearcherV37::quiescence(
    Position& pos,
    int alpha,
    int beta,
    int ply,
...
NnueSearcherV37::SearchValue NnueSearcherV37::negamax(
    Position& pos,
    int depth,
    int ply,
    Move prev_move,
...
SearchResult NnueSearcherV37::make_fallback_result(const Position& pos) const {
    SearchResult result;
...
NnueSearcherV37::RootSearchResult NnueSearcherV37::search_fixed_depth(
    Position pos,
    int depth,
    SearchState& state,
    int alpha,
...
SearchResult NnueSearcherV37::search_root_without_tt_probe(
    const Position& pos,
    int depth,
    SearchState& state
) {
...
SearchResult NnueSearcherV37::search_best_move(const Position& pos, int depth) {
    killer_table_.clear();
...
SearchResult NnueSearcherV37::search_best_move(const Position& pos, const SearchLimits& limits) {
