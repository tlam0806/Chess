...
int HeuristicSearcherV29::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV29::TTProbeResult HeuristicSearcherV29::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV29::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void HeuristicSearcherV29::update_best_range(
...
void HeuristicSearcherV29::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
