...
struct HeuristicSearcherV23::SearchState {
...
enum class HeuristicSearcherV23::ScoringMode {
    MainSearch,
    Quiescence
};

enum class HeuristicSearcherV23::MoveGenerationStage {
    Noisy,
    QuietNonPromotion
};

struct HeuristicSearcherV23::SearchValue {
    ScoreRange range{};
};

struct HeuristicSearcherV23::TTProbeResult {
    ScoreRange range{};
    MoveRange moves{};
    bool hit = false;
...
int HeuristicSearcherV23::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV23::TTProbeResult HeuristicSearcherV23::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV23::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void HeuristicSearcherV23::update_best_range(
...
void HeuristicSearcherV23::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
...
void HeuristicSearcherV23::reward_quiet_cutoff(
    Color side_to_move,
    int depth,
    int ply,
...
void HeuristicSearcherV23::penalize_failed_quiets(
    Color side_to_move,
    int depth,
    Move prev_move,
    PieceType prev_moved_piece,
...
bool HeuristicSearcherV23::should_stop(SearchState& state) const {
...
int HeuristicSearcherV23::move_order_score(
    Move move,
    const Position& pos,
...
HeuristicSearcherV23::ScoredMove HeuristicSearcherV23::make_scored_move(
    const Position& pos,
...
bool HeuristicSearcherV23::better_scored_move(
...
void HeuristicSearcherV23::sort_scored_moves(std::vector<ScoredMove>& moves) const {
...
std::vector<HeuristicSearcherV23::ScoredMove> HeuristicSearcherV23::ordered_moves(
    const Position& pos,
    int ply,
    MoveRange tt_moves,
    Move prev_move,
...
std::vector<HeuristicSearcherV23::ScoredMove> HeuristicSearcherV23::ordered_moves_for_stage(
    const Position& pos,
    int ply,
    MoveRange tt_moves,
    Move prev_move,
    ...
    const KingSafetyContext king_safety = make_king_safety_context(pos);
    MoveList moves;
    if (generation_stage == MoveGenerationStage::Noisy) {
...
HeuristicSearcherV23::SearchValue HeuristicSearcherV23::quiescence(
    Position pos,
    int alpha,
    int beta,
    int ply,
...
bool HeuristicSearcherV23::is_quiet_move(const ScoredMove& scored_move) const {
    return !scored_move.capture
        && !scored_move.promotion
        && !scored_move.gives_check;
}

void HeuristicSearcherV23::add_move_type_stats(
    MoveTypeStats& stats,
...
void HeuristicSearcherV23::record_node_with_moves(ScoringMode stage) {
    if (!move_ordering_stats_enabled_) {
        return;
    }
...
void HeuristicSearcherV23::record_beta_cutoff(
...
void HeuristicSearcherV23::record_generated_stage_move(
    const ScoredMove& move,
    Color side_to_move,
    int ply,
    MoveRange tt_moves,
    ...
    MoveTypeStats& stats = generation_stage == MoveGenerationStage::Noisy
...
HeuristicSearcherV23::SearchValue HeuristicSearcherV23::negamax(
    Position pos,
    int depth,
    int ply,
    Move prev_move,
        ...
        for (std::size_t move_index = 0; move_index < moves.size(); ++move_index) {
            ...
            if (node_range.lower >= beta) {
                ...
                const bool has_unsearched_stage =
                    generation_stage == MoveGenerationStage::Noisy
...
SearchResult HeuristicSearcherV23::make_fallback_result(const Position& pos) const {
    SearchResult result;
...
SearchResult HeuristicSearcherV23::search_fixed_depth(
    const Position& pos,
    int depth,
    SearchState& state,
    int alpha,
...
SearchResult HeuristicSearcherV23::search_root_without_tt_probe(
    const Position& pos,
    int depth,
    SearchState& state
) {
...
SearchResult HeuristicSearcherV23::search_best_move(const Position& pos, int depth) {
    killer_table_.clear();
    SearchState state;
    return search_fixed_depth(pos, depth, state);
}

SearchResult HeuristicSearcherV23::search_best_move(const Position& pos, const SearchLimits& limits) {
