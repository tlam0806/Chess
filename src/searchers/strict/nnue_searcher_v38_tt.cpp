...
int NnueSearcherV38::score_to_tt(int score, int ply) const {
...
NnueSearcherV38::TTProbeResult NnueSearcherV38::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool NnueSearcherV38::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void NnueSearcherV38::update_best_range(
...
void NnueSearcherV38::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
