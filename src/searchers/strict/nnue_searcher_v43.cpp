...
int adaptive_aspiration_delta(
    const NnueSearcherV43::AspirationConfig& config,
    int mean_score
) {
...
bool NnueSearcherV43::can_null_move_prune(
    const Position& pos,
    int depth,
...
void NnueSearcherV43::make_null_move(Position& pos) {
...
bool NnueSearcherV43::castling_rights_changed(
...
void NnueSearcherV43::initialize_repetition(
    SearchState& state,
    const Position& pos,
...
bool NnueSearcherV43::history_draw(
    const Position& pos,
    SearchState& state
) const {
...
bool NnueSearcherV43::allow_repetition_tt_score(SearchState& state) const {
...
void NnueSearcherV43::reward_quiet_cutoff(
    Color side_to_move,
    int depth,
    int ply,
...
void NnueSearcherV43::penalize_failed_quiets(
    Color side_to_move,
    int depth,
    Move prev_move,
    PieceType prev_moved_piece,
    const NnueSearcherV43::ScoredMoveList& failed_quiet_moves
) {
...
void NnueSearcherV43::record_beta_cutoff(
    const ScoredMove& scored_move,
...
bool NnueSearcherV43::should_stop(
    SearchState& state,
    bool force_poll
) const {
...
KingSafetyContext NnueSearcherV43::current_king_safety_context(const Position& pos) const {
    return cached_king_safety_context(pos, pos.side_to_move);
}

int NnueSearcherV43::evaluate_current_position(const Position& pos, const SearchState& state) const {
...
NnueSearcherV43::SearchValue NnueSearcherV43::quiescence(
    Position& pos,
    int alpha,
    int beta,
    int ply,
...
NnueSearcherV43::SearchValue NnueSearcherV43::negamax(
    Position& pos,
    int depth,
    int ply,
    Move prev_move,
...
SearchResult NnueSearcherV43::make_fallback_result(const Position& pos) const {
    SearchResult result;
...
NnueSearcherV43::RootSearchResult NnueSearcherV43::search_fixed_depth(
    Position pos,
    int depth,
    SearchState& state,
    int alpha,
    ...
    if (depth == 0) {
        ...
        root_result.bound = V43SingleBoundTranspositionTable::Bound::Exact;
    ...
    if (best_score <= original_alpha) {
        root_result.bound = V43SingleBoundTranspositionTable::Bound::Upper;
    } else if (best_score >= original_beta) {
        root_result.bound = V43SingleBoundTranspositionTable::Bound::Lower;
    } else {
        root_result.bound = V43SingleBoundTranspositionTable::Bound::Exact;
    }
...
SearchResult NnueSearcherV43::search_best_move(const Position& pos, int depth) {
    return search_best_move_impl(pos, depth, {}, true);
}

SearchResult NnueSearcherV43::search_best_move(
    const Position& pos,
    int depth,
    std::span<const HashKey> game_history
) {
...
SearchResult NnueSearcherV43::search_best_move_impl(
    const Position& pos,
    int depth,
    std::span<const HashKey> game_history,
...
SearchResult NnueSearcherV43::search_best_move(const Position& pos, const SearchLimits& limits) {
...
SearchResult NnueSearcherV43::search_best_move_impl(
    ...
    auto record_completed_iteration = [&](const RootSearchResult& current) {
        if (current.bound != V43SingleBoundTranspositionTable::Bound::Exact) {
            return;
        }
    ...
    for (int depth = 1; depth <= limits.max_depth; ++depth) {
        ...
        // V43 polls both its clock and cooperative stop at every root
        ...
        if (aspiration_config_.enabled) {
            ...
            if (!use_narrow_window) {
                ...
                if (current.bound
                    != V43SingleBoundTranspositionTable::Bound::Exact) {
                ...
                while (true) {
                    ...
                    if (current.bound
                        == V43SingleBoundTranspositionTable::Bound::Upper) {
                        ...
                        if (value <= -MateScoreThreshold) {
                            alpha = -Infinity;
                               ...
                               == V43SingleBoundTranspositionTable::Bound::Lower) {
