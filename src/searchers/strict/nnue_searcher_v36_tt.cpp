...
int NnueSearcherV36::score_to_tt(int score, int ply) const {
...
NnueSearcherV36::TTProbeResult NnueSearcherV36::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool NnueSearcherV36::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void NnueSearcherV36::update_best_range(
...
void NnueSearcherV36::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
