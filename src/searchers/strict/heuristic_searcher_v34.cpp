...
void HeuristicSearcherV34::reward_quiet_cutoff(
    Color side_to_move,
    int depth,
    int ply,
...
void HeuristicSearcherV34::penalize_failed_quiets(
    Color side_to_move,
    int depth,
    Move prev_move,
    PieceType prev_moved_piece,
    const HeuristicSearcherV34::ScoredMoveList& failed_quiet_moves
) {
...
bool HeuristicSearcherV34::should_stop(SearchState& state) const {
...
KingSafetyContext HeuristicSearcherV34::current_king_safety_context(const Position& pos) const {
    return cached_king_safety_context(pos, pos.side_to_move);
}

int HeuristicSearcherV34::evaluate_current_position(const Position& pos) const {
    return evaluate_for_side_to_move(pos);
}



HeuristicSearcherV34::SearchValue HeuristicSearcherV34::quiescence(
    Position& pos,
    int alpha,
    int beta,
    int ply,
...
HeuristicSearcherV34::SearchValue HeuristicSearcherV34::negamax(
    Position& pos,
    int depth,
    int ply,
    Move prev_move,
...
SearchResult HeuristicSearcherV34::make_fallback_result(const Position& pos) const {
    SearchResult result;
...
HeuristicSearcherV34::RootSearchResult HeuristicSearcherV34::search_fixed_depth(
    Position pos,
    int depth,
    SearchState& state,
    int alpha,
...
SearchResult HeuristicSearcherV34::search_root_without_tt_probe(
    const Position& pos,
    int depth,
    SearchState& state
) {
...
SearchResult HeuristicSearcherV34::search_best_move(const Position& pos, int depth) {
    killer_table_.clear();
...
SearchResult HeuristicSearcherV34::search_best_move(const Position& pos, const SearchLimits& limits) {
