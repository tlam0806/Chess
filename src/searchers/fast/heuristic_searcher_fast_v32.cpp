...
void HeuristicSearcherFastV32::reward_quiet_cutoff(
    Color side_to_move,
    int depth,
    int ply,
...
void HeuristicSearcherFastV32::penalize_failed_quiets(
    Color side_to_move,
    int depth,
    Move prev_move,
    PieceType prev_moved_piece,
    const HeuristicSearcherFastV32::ScoredMoveList& failed_quiet_moves
) {
...
bool HeuristicSearcherFastV32::should_stop(SearchState& state) const {
...
KingSafetyContext HeuristicSearcherFastV32::current_king_safety_context(const Position& pos) const {
    return cached_king_safety_context(pos, pos.side_to_move);
}

int HeuristicSearcherFastV32::evaluate_current_position(const Position& pos) const {
    return evaluate_for_side_to_move(pos);
}



HeuristicSearcherFastV32::SearchValue HeuristicSearcherFastV32::quiescence(
    Position& pos,
    int alpha,
    int beta,
    int ply,
...
HeuristicSearcherFastV32::SearchValue HeuristicSearcherFastV32::negamax(
    Position& pos,
    int depth,
    int ply,
    Move prev_move,
    ...
    auto search_one_scored_move =
        ...
        ScoreRange move_range;
        {
            ...
            const bool can_lmr =
                lmr_config_.enabled
                && depth > 2
                && depth >= lmr_config_.min_depth
                && move_index >= lmr_config_.min_move_index
                && lmr_config_.divisor > 0.0
                && king_safety_cache
            ...
            if (can_lmr) {
                int reduction = static_cast<int>(
                    lmr_config_.base + std::log(static_cast<double>(depth))
                        * std::log(static_cast<double>(move_index + 1)) / lmr_config_.divisor);
    ...
    auto search_scored_moves = [&]<MoveGenerationStage Stage>(const ScoredMoveList& moves) {
        ...
        for (std::size_t move_index = 0; move_index < moves.size(); ++move_index) {
            ...
            search_one_scored_move.template operator()<Stage>(moves[move_index], move_index, has_unsearched_stage);
            if (cutoff || stage_stopped) {
                return;
            }
        }
        ...
        auto search_promotion_move = [&](Move move, PieceType moved_piece, PieceType captured_piece) {
            ...
            const ScoredMove scored_move = captured_piece == PieceType::None
                ...
                : make_scored_legal_move<ScoringMode::MainSearch, true, true>(
                    pos, move, moved_piece, captured_piece, ply, tt_probe.moves, prev_move, prev_moved_piece);
            search_one_scored_move.template operator()<MoveGenerationStage::Promotion>(scored_move, 0, true);
        };
...
SearchResult HeuristicSearcherFastV32::make_fallback_result(const Position& pos) const {
    SearchResult result;
...
HeuristicSearcherFastV32::RootSearchResult HeuristicSearcherFastV32::search_fixed_depth(
    Position pos,
    int depth,
    SearchState& state,
    int alpha,
...
SearchResult HeuristicSearcherFastV32::search_root_without_tt_probe(
    const Position& pos,
    int depth,
    SearchState& state
) {
...
SearchResult HeuristicSearcherFastV32::search_best_move(const Position& pos, int depth) {
    killer_table_.clear();
...
SearchResult HeuristicSearcherFastV32::search_best_move(const Position& pos, const SearchLimits& limits) {
