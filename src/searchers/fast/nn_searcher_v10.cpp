...
void NnSearcherV10::make_null_move(Position& pos) const {
...
bool NnSearcherV10::should_stop(SearchContext& context) const {
...
NnSearcherV10::MoveOrderStage NnSearcherV10::move_order_stage(
    Move move,
    Move tt_move,
    int ply,
    bool gives_check,
...
int NnSearcherV10::move_tie_break_score(
    Move move,
    const Position& pos,
    int ply,
    bool gives_check,
...
NnSearcherV10::ScoredMove NnSearcherV10::make_scored_move(
    const Position& pos,
    Move move,
    int ply,
    Move tt_move,
...
bool NnSearcherV10::better_scored_move(
...
int NnSearcherV10::stage_rank(MoveOrderStage stage) const {
    switch (profile_) {
...
std::vector<NnSearcherV10::ScoredMove> NnSearcherV10::ordered_moves(
    const Position& pos,
    int ply,
    Move tt_move
) const {
...
std::vector<NnSearcherV10::ScoredMove> NnSearcherV10::ordered_noisy_moves(
    const Position& pos
) const {
    std::vector<ScoredMove> ordered;
    const std::vector<Move> moves = generate_legal_moves(pos);
...
bool NnSearcherV10::should_reduce_late_move(
    const Position& pos,
...
bool NnSearcherV10::can_null_move_prune(
    const Position& pos,
    int depth,
...
int NnSearcherV10::quiescence(
    ...
    const int stand_pat = model_.evaluate_cp_rounded(pos);
    if (stand_pat >= beta) {
        return beta;
    }
    if (stand_pat > alpha) {
...
bool NnSearcherV10::is_quiet_move(const ScoredMove& scored_move) const {
    return !scored_move.capture
        && !scored_move.promotion
        && !scored_move.gives_check;
}

int NnSearcherV10::negamax(
    Position pos,
    int depth,
    int ply,
    int alpha,
...
SearchResult NnSearcherV10::make_fallback_result(const Position& pos) const {
    SearchResult result;
...
SearchResult NnSearcherV10::search_fixed_depth(
    const Position& pos,
    int depth,
    SearchContext& context
) {
...
SearchResult NnSearcherV10::search_best_move(const Position& pos, int depth) {
    killer_table_.clear();
    SearchContext context;
    return search_fixed_depth(pos, depth, context);
}

SearchResult NnSearcherV10::search_best_move(const Position& pos, const SearchLimits& limits) {
...
std::string_view NnSearcherV10::name() const {
    return "nn_v10";
}

void NnSearcherV10::clear_tt() {
    tt_.clear();
}

std::size_t NnSearcherV10::tt_entry_count() const {
    return tt_.entry_count();
}

void NnSearcherV10::clear_tt_stats() {
    tt_.clear_stats();
}

const TranspositionTableStats& NnSearcherV10::tt_stats() const {
    return tt_.stats();
}
