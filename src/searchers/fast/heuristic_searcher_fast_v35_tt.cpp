...
int HeuristicSearcherFastV35::score_to_tt(int score, int ply) const {
...
HeuristicSearcherFastV35::TTProbeResult HeuristicSearcherFastV35::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherFastV35::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void HeuristicSearcherFastV35::update_best_range(
...
void HeuristicSearcherFastV35::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
