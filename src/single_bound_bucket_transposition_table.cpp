...
using SingleTT = SingleBoundBucketTranspositionTable;
using Bound = SingleTT::Bound;
...
std::size_t entry_count_from_megabytes(std::size_t megabytes) {
    ...
    const std::size_t bytes_per_entry = sizeof(SingleTT::KeyLock) + sizeof(SingleTT::TTValue);
...
int replacement_priority(const SingleTT::TTValue& value) {
...
SingleTT::KeyLock make_tt_lock(HashKey key) {
    assert(key != 0);
    const auto lock = static_cast<SingleTT::KeyLock>(key >> 32);
    return lock == 0 ? SingleTT::KeyLock{1} : lock;
}
...
SingleTT::TTValue to_tt_value(StoreValue value) {
    return SingleTT::TTValue{value.score, value.move, value.depth, value.bound};
}
...
bool should_replace_same_key(const SingleTT::TTValue& old_value, StoreValue new_value) {
...
SingleBoundBucketTranspositionTable::SingleBoundBucketTranspositionTable(
...
void SingleBoundBucketTranspositionTable::clear() {
...
void SingleBoundBucketTranspositionTable::clear_stats() {
    stats_ = {};
}

std::size_t SingleBoundBucketTranspositionTable::entry_count() const {
    return keys_.size();
}

std::size_t SingleBoundBucketTranspositionTable::bucket_size() const {
    return bucket_size_;
}

std::size_t SingleBoundBucketTranspositionTable::bucket_count() const {
    return bucket_count_;
}

const RangeTranspositionTableStats& SingleBoundBucketTranspositionTable::stats() const {
    return stats_;
}

#ifdef CHESS_PROFILE_TT_TIMING
void SingleBoundBucketTranspositionTable::clear_timing_stats() {
    timing_stats_ = {};
}

const TTFunctionTimingStats& SingleBoundBucketTranspositionTable::timing_stats() const {
    return timing_stats_;
}
#endif

#ifdef CHESS_PROFILE_TT_PATH_TIMING
void SingleBoundBucketTranspositionTable::clear_probe_path_timing_stats() {
    probe_path_timing_stats_ = {};
}

const TTProbePathTimingStats&
SingleBoundBucketTranspositionTable::probe_path_timing_stats() const {
...
void accumulate_cache_line_sample(
    SingleTT::ProbeCacheLineStats& stats,
    const SingleTT::ProbeCacheLineSample& sample
) {
    ++stats.probes;
...
SingleBoundBucketTranspositionTable::ProbeCacheLineSample
SingleBoundBucketTranspositionTable::probe_cache_line_sample(
    HashKey key,
...
void SingleBoundBucketTranspositionTable::clear_cache_line_stats() {
    cache_line_stats_ = {};
}

const SingleBoundBucketTranspositionTable::ProbeCacheLineStats&
SingleBoundBucketTranspositionTable::cache_line_stats() const {
    return cache_line_stats_;
}
#endif

std::size_t SingleBoundBucketTranspositionTable::bucket_offset(HashKey key) const {
    return (key & bucket_mask_) * bucket_size_;
}

bool SingleBoundBucketTranspositionTable::probe(
    HashKey key,
    int depth,
...
#endif
    assert(depth_policy == TTDepthPolicy::AtLeast);
    (void)depth_policy;
    ...
    auto probe_hit = [&](std::size_t index) {
        ...
        if (static_cast<int>(value.depth) < depth) {
...
void SingleBoundBucketTranspositionTable::store(
    HashKey key,
    int depth,
    ScoreRange score,
    MoveRange move
