...
NnSearcherV6::NnSearcherV6(const NnValueModel& model, std::size_t tt_mb)
    : model_(model),
      tt_(tt_mb) {
}

SearchResult NnSearcherV6::search_best_move(const Position& pos, int depth) {
    SearchContext context;
    return search_fixed_depth(pos, depth, tt_, context, model_);
}

SearchResult NnSearcherV6::search_best_move(const Position& pos, const SearchLimits& limits) {
...
std::string_view NnSearcherV6::name() const {
    return "nn_v6";
}

void NnSearcherV6::clear_tt() {
    tt_.clear();
}

std::size_t NnSearcherV6::tt_entry_count() const {
