...
`tools/tune/run_nnue_v45_pareto_tune.sh` and
`tools/match/run_nnue_v45_round_robin.sh`.
...
| `apps/dataset-viewer/` | Local browser UI for playing, replaying logged games and inspecting dataset samples. |
...
## Historical dense-model prototype

Before the current NNUE pipeline, the project used a small dense value network
and a standalone neural search engine. Its dataset export, training, C++
inference and CLI commands are preserved in the
[dense-model prototype guide](docs/history/dense-model-prototype.md).
