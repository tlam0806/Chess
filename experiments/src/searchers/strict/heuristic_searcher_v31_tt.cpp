...
int HeuristicSearcherV31::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV31::TTProbeResult HeuristicSearcherV31::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV31::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void HeuristicSearcherV31::update_best_range(
...
void HeuristicSearcherV31::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
