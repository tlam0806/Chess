...
constexpr int TtLowerBonus = 1'000'000'000;
constexpr int TtUpperBonus = 20'000'000;
...
struct HeuristicSearcherV16::SearchState {
...
enum class HeuristicSearcherV16::ScoringMode {
    MainSearch,
    Quiescence
};

struct HeuristicSearcherV16::ScoredMove {
    Move move{};
    int order_score = 0;
...
struct HeuristicSearcherV16::SearchValue {
    ScoreRange range{};
};

struct HeuristicSearcherV16::TTProbeResult {
    ScoreRange range{};
    MoveRange moves{};
    bool hit = false;
...
int HeuristicSearcherV16::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV16::TTProbeResult HeuristicSearcherV16::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV16::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

bool HeuristicSearcherV16::should_stop(SearchState& state) const {
...
int HeuristicSearcherV16::move_order_score(
    Move move,
    const Position& pos,
    int ply,
    MoveRange tt_moves,
...
HeuristicSearcherV16::ScoredMove HeuristicSearcherV16::make_scored_move(
    const Position& pos,
    Move move,
    int ply,
    MoveRange tt_moves,
...
bool HeuristicSearcherV16::better_scored_move(
...
std::vector<HeuristicSearcherV16::ScoredMove> HeuristicSearcherV16::ordered_moves(
    const Position& pos,
    int ply,
    MoveRange tt_moves
) const {
    ...
    for (Move move : moves) {
        ordered.push_back(make_scored_move(pos, move, ply, tt_moves, ScoringMode::MainSearch));
    }
...
std::vector<HeuristicSearcherV16::ScoredMove> HeuristicSearcherV16::ordered_noisy_moves(
    const Position& pos
) const {
    std::vector<ScoredMove> ordered;
    const std::vector<Move> moves = generate_legal_moves(pos);
    ...
    for (Move move : moves) {
        ...
        ordered.push_back(make_scored_move(pos, move, 0, MoveRange{}, ScoringMode::Quiescence));
    }
...
HeuristicSearcherV16::SearchValue HeuristicSearcherV16::quiescence(
    Position pos,
    int alpha,
    int beta,
    int ply,
    ...
    if (in_check(pos, pos.side_to_move)) {
        ...
        for (Move move : legal_moves) {
            moves.push_back(make_scored_move(pos, move, ply, MoveRange{}, ScoringMode::Quiescence));
        }
...
bool HeuristicSearcherV16::is_quiet_move(const ScoredMove& scored_move) const {
    return !scored_move.capture
        && !scored_move.promotion
        && !scored_move.gives_check;
}

HeuristicSearcherV16::SearchValue HeuristicSearcherV16::negamax(
    Position pos,
    int depth,
    int ply,
    int alpha,
    ...
    const std::vector<ScoredMove> moves = ordered_moves(pos, ply, tt_probe.moves);
    ...
    if (best_index < moves.size()) {
        ...
        if (best_index == 0) {
            ...
            ++state.move_ordering_stats.missed_predicted_categories[
                static_cast<std::size_t>(moves.front().category)
            ];
            ++state.move_ordering_stats.missed_best_categories[
                static_cast<std::size_t>(moves[best_index].category)
            ];
        }
    }
...
SearchResult HeuristicSearcherV16::make_fallback_result(const Position& pos) const {
    SearchResult result;
...
SearchResult HeuristicSearcherV16::search_fixed_depth(
    const Position& pos,
    int depth,
    SearchState& state,
    int alpha,
...
SearchResult HeuristicSearcherV16::search_root_without_tt_probe(
    const Position& pos,
    int depth,
    SearchState& state
) {
...
HeuristicSearcherV16::HeuristicSearcherV16(
    std::size_t tt_mb,
...
SearchResult HeuristicSearcherV16::search_best_move(const Position& pos, int depth) {
    killer_table_.clear();
    SearchState state;
...
SearchResult HeuristicSearcherV16::search_best_move(const Position& pos, const SearchLimits& limits) {
...
std::string_view HeuristicSearcherV16::name() const {
    return "heuristic_v16";
}

void HeuristicSearcherV16::clear_tt() {
    tt_.clear();
}

std::size_t HeuristicSearcherV16::tt_entry_count() const {
    return tt_.entry_count();
}

void HeuristicSearcherV16::clear_tt_stats() {
    tt_.clear_stats();
}

const RangeTranspositionTableStats& HeuristicSearcherV16::tt_stats() const {
    return tt_.stats();
}

void HeuristicSearcherV16::clear_move_ordering_stats() {
    move_ordering_stats_ = {};
}

const HeuristicSearcherV16::MoveOrderingStats& HeuristicSearcherV16::move_ordering_stats() const {
