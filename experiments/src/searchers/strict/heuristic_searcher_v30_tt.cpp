...
int HeuristicSearcherV30::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV30::TTProbeResult HeuristicSearcherV30::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV30::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void HeuristicSearcherV30::update_best_range(
...
void HeuristicSearcherV30::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
