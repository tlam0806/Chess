...
struct HeuristicSearcherV17::SearchState {
...
enum class HeuristicSearcherV17::ScoringMode {
    MainSearch,
    Quiescence
};

struct HeuristicSearcherV17::ScoredMove {
    Move move{};
    int order_score = 0;
...
struct HeuristicSearcherV17::SearchValue {
    ScoreRange range{};
};

struct HeuristicSearcherV17::TTProbeResult {
    ScoreRange range{};
    MoveRange moves{};
    bool hit = false;
...
int HeuristicSearcherV17::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV17::TTProbeResult HeuristicSearcherV17::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV17::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

bool HeuristicSearcherV17::should_stop(SearchState& state) const {
...
int HeuristicSearcherV17::move_order_score(
    Move move,
    const Position& pos,
    int ply,
    MoveRange tt_moves,
...
HeuristicSearcherV17::ScoredMove HeuristicSearcherV17::make_scored_move(
    const Position& pos,
    Move move,
    int ply,
    MoveRange tt_moves,
...
bool HeuristicSearcherV17::better_scored_move(
...
std::vector<HeuristicSearcherV17::ScoredMove> HeuristicSearcherV17::ordered_moves(
    const Position& pos,
    int ply,
    MoveRange tt_moves,
    Move prev_move,
...
std::vector<HeuristicSearcherV17::ScoredMove> HeuristicSearcherV17::ordered_noisy_moves(
    const Position& pos
) const {
    std::vector<ScoredMove> ordered;
    const std::vector<Move> moves = generate_legal_moves(pos);
...
HeuristicSearcherV17::SearchValue HeuristicSearcherV17::quiescence(
    Position pos,
    int alpha,
    int beta,
    int ply,
...
bool HeuristicSearcherV17::is_quiet_move(const ScoredMove& scored_move) const {
    return !scored_move.capture
        && !scored_move.promotion
        && !scored_move.gives_check;
}

HeuristicSearcherV17::SearchValue HeuristicSearcherV17::negamax(
    Position pos,
    int depth,
    int ply,
    Move prev_move,
...
SearchResult HeuristicSearcherV17::make_fallback_result(const Position& pos) const {
    SearchResult result;
...
SearchResult HeuristicSearcherV17::search_fixed_depth(
    const Position& pos,
    int depth,
    SearchState& state,
    int alpha,
...
SearchResult HeuristicSearcherV17::search_root_without_tt_probe(
    const Position& pos,
    int depth,
    SearchState& state
) {
...
SearchResult HeuristicSearcherV17::search_best_move(const Position& pos, int depth) {
    killer_table_.clear();
    SearchState state;
...
SearchResult HeuristicSearcherV17::search_best_move(const Position& pos, const SearchLimits& limits) {
...
std::string_view HeuristicSearcherV17::name() const {
    return "heuristic_v17";
}

void HeuristicSearcherV17::clear_tt() {
    tt_.clear();
}

std::size_t HeuristicSearcherV17::tt_entry_count() const {
    return tt_.entry_count();
}

void HeuristicSearcherV17::clear_tt_stats() {
    tt_.clear_stats();
}

const RangeTranspositionTableStats& HeuristicSearcherV17::tt_stats() const {
    return tt_.stats();
}

void HeuristicSearcherV17::clear_move_ordering_stats() {
    move_ordering_stats_ = {};
}

const HeuristicSearcherV17::MoveOrderingStats& HeuristicSearcherV17::move_ordering_stats() const {
