...
int HeuristicSearcherV28::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV28::TTProbeResult HeuristicSearcherV28::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV28::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void HeuristicSearcherV28::update_best_range(
...
void HeuristicSearcherV28::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
