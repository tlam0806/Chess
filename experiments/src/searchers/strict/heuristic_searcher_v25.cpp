...
struct HeuristicSearcherV25::SearchState {
...
enum class HeuristicSearcherV25::ScoringMode {
    MainSearch,
    Quiescence
};

enum class HeuristicSearcherV25::MoveGenerationStage {
    Priority,
    QuietNonPromotion
};

struct HeuristicSearcherV25::SearchValue {
    ScoreRange range{};
};

struct HeuristicSearcherV25::RootSearchResult {
    SearchResult result{};
    ScoreRange range{};
};

struct HeuristicSearcherV25::TTProbeResult {
    ScoreRange range{};
    MoveRange moves{};
    bool hit = false;
...
int HeuristicSearcherV25::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV25::TTProbeResult HeuristicSearcherV25::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV25::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void HeuristicSearcherV25::update_best_range(
...
void HeuristicSearcherV25::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
...
void HeuristicSearcherV25::reward_quiet_cutoff(
    Color side_to_move,
    int depth,
    int ply,
...
void HeuristicSearcherV25::penalize_failed_quiets(
    Color side_to_move,
    int depth,
    Move prev_move,
    PieceType prev_moved_piece,
    const HeuristicSearcherV25::ScoredMoveList& failed_quiet_moves
) {
...
bool HeuristicSearcherV25::should_stop(SearchState& state) const {
...
KingSafetyContext HeuristicSearcherV25::profiled_king_safety_context(const Position& pos) const {
    ProfileTimer timer(
        timing_stats_enabled_,
...
int HeuristicSearcherV25::profiled_evaluate_for_side_to_move(const Position& pos) const {
    ProfileTimer timer(
        timing_stats_enabled_,
...
Position HeuristicSearcherV25::profiled_child_position(
    const Position& pos,
    const ScoredMove& scored_move
) const {
    ProfileTimer timer(
...
int HeuristicSearcherV25::move_order_score(
    Move move,
    const Position& pos,
    PieceType moved_piece,
    int ply,
...
HeuristicSearcherV25::ScoredMove HeuristicSearcherV25::make_scored_move(
    const Position& pos,
...
HeuristicSearcherV25::ScoredMove HeuristicSearcherV25::make_scored_move_by_attack_check(
    const Position& pos,
    Move move,
    int ply,
    MoveRange tt_moves,
...
HeuristicSearcherV25::ScoredMove HeuristicSearcherV25::make_scored_legal_move(
    const Position& pos,
    Move move,
    PieceType moved_piece,
    PieceType captured_piece,
...
bool HeuristicSearcherV25::better_scored_move(
...
void HeuristicSearcherV25::sort_scored_moves(ScoredMoveList& moves) const {
    ProfileTimer timer(
        timing_stats_enabled_,
...
HeuristicSearcherV25::ScoredMoveList HeuristicSearcherV25::ordered_moves(
    const Position& pos,
    int ply,
    MoveRange tt_moves,
    Move prev_move,
...
Move HeuristicSearcherV25::valid_priority_killer(const Position& pos, int ply, int slot) const {
...
HeuristicSearcherV25::ScoredMoveList
HeuristicSearcherV25::generate_legal_noisy_scored_moves_for_searcher(
    const Position& pos,
    const KingSafetyContext& king_safety,
    int ply,
    MoveRange tt_moves,
...
HeuristicSearcherV25::ScoredMoveList HeuristicSearcherV25::ordered_moves_for_stage(
    const Position& pos,
    const KingSafetyContext& king_safety,
    int ply,
    MoveRange tt_moves,
...
HeuristicSearcherV25::SearchValue HeuristicSearcherV25::quiescence(
    Position pos,
    int alpha,
    int beta,
    int ply,
    ...
    if (side_in_check) {
        ...
        for (std::size_t move_index = 0; move_index < moves.size(); ++move_index) {
            ...
            Position next = profiled_child_position(pos, scored_move);
...
bool HeuristicSearcherV25::is_quiet_move(const ScoredMove& scored_move) const {
    return !scored_move.capture
        && !scored_move.promotion
        && !scored_move.gives_check;
}

void HeuristicSearcherV25::add_move_type_stats(
    MoveTypeStats& stats,
...
void HeuristicSearcherV25::record_node_with_moves(ScoringMode stage) {
    if (!move_ordering_stats_enabled_) {
        return;
    }
...
void HeuristicSearcherV25::record_beta_cutoff(
    const HeuristicSearcherV25::ScoredMoveList& moves,
...
void HeuristicSearcherV25::record_generated_stage_move(
    const ScoredMove& move,
    Color side_to_move,
    int ply,
    MoveRange tt_moves,
...
void HeuristicSearcherV25::record_priority_stage_top_moves(
    const HeuristicSearcherV25::ScoredMoveList& moves,
    Color side_to_move,
    int ply,
    MoveRange tt_moves,
...
HeuristicSearcherV25::SearchValue HeuristicSearcherV25::negamax(
    Position pos,
    int depth,
    int ply,
    Move prev_move,
...
SearchResult HeuristicSearcherV25::make_fallback_result(const Position& pos) const {
    SearchResult result;
...
HeuristicSearcherV25::RootSearchResult HeuristicSearcherV25::search_fixed_depth(
    const Position& pos,
    int depth,
    SearchState& state,
    int alpha,
...
SearchResult HeuristicSearcherV25::search_root_without_tt_probe(
    const Position& pos,
    int depth,
    SearchState& state
) {
...
SearchResult HeuristicSearcherV25::search_best_move(const Position& pos, int depth) {
    killer_table_.clear();
    SearchState state;
    return search_fixed_depth(pos, depth, state).result;
}

SearchResult HeuristicSearcherV25::search_best_move(const Position& pos, const SearchLimits& limits) {
