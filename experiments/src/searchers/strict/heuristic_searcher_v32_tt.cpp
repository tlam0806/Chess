...
int HeuristicSearcherV32::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV32::TTProbeResult HeuristicSearcherV32::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV32::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void HeuristicSearcherV32::update_best_range(
...
void HeuristicSearcherV32::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
