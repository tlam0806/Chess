...
constexpr bool EnableAspirationWindow = false;
...
struct HeuristicSearcherV15Pvs::SearchState {
...
enum class HeuristicSearcherV15Pvs::ScoringMode {
    MainSearch,
    Quiescence
};

struct HeuristicSearcherV15Pvs::ScoredMove {
    Move move{};
    int order_score = 0;
...
struct HeuristicSearcherV15Pvs::SearchValue {
    ScoreRange range{};
};

struct HeuristicSearcherV15Pvs::TTProbeResult {
    ScoreRange range{};
    MoveRange moves{};
    bool hit = false;
...
int HeuristicSearcherV15Pvs::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV15Pvs::TTProbeResult HeuristicSearcherV15Pvs::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV15Pvs::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

bool HeuristicSearcherV15Pvs::should_stop(SearchState& state) const {
...
int HeuristicSearcherV15Pvs::move_order_score(
    Move move,
    const Position& pos,
    int ply,
    Move tt_move,
...
HeuristicSearcherV15Pvs::ScoredMove HeuristicSearcherV15Pvs::make_scored_move(
    const Position& pos,
    Move move,
    int ply,
    Move tt_move,
...
bool HeuristicSearcherV15Pvs::better_scored_move(
...
std::vector<HeuristicSearcherV15Pvs::ScoredMove> HeuristicSearcherV15Pvs::ordered_moves(
    const Position& pos,
    int ply,
    Move tt_move
) const {
...
std::vector<HeuristicSearcherV15Pvs::ScoredMove> HeuristicSearcherV15Pvs::ordered_noisy_moves(
    const Position& pos
) const {
    std::vector<ScoredMove> ordered;
    const std::vector<Move> moves = generate_legal_moves(pos);
...
HeuristicSearcherV15Pvs::SearchValue HeuristicSearcherV15Pvs::quiescence(
    Position pos,
    int alpha,
    int beta,
    int ply,
...
bool HeuristicSearcherV15Pvs::is_quiet_move(const ScoredMove& scored_move) const {
    return !scored_move.capture
        && !scored_move.promotion
        && !scored_move.gives_check;
}

HeuristicSearcherV15Pvs::SearchValue HeuristicSearcherV15Pvs::negamax(
    Position pos,
    int depth,
    int ply,
    int alpha,
...
SearchResult HeuristicSearcherV15Pvs::make_fallback_result(const Position& pos) const {
    SearchResult result;
...
SearchResult HeuristicSearcherV15Pvs::search_fixed_depth(
    const Position& pos,
    int depth,
    SearchState& state,
    int alpha,
...
SearchResult HeuristicSearcherV15Pvs::search_root_without_tt_probe(
    const Position& pos,
    int depth,
    SearchState& state
) {
...
HeuristicSearcherV15Pvs::HeuristicSearcherV15Pvs(std::size_t tt_mb)
    : tt_(tt_mb) {
}

SearchResult HeuristicSearcherV15Pvs::search_best_move(const Position& pos, int depth) {
    killer_table_.clear();
    SearchState state;
    return search_fixed_depth(pos, depth, state);
}

SearchResult HeuristicSearcherV15Pvs::search_best_move(const Position& pos, const SearchLimits& limits) {
...
std::string_view HeuristicSearcherV15Pvs::name() const {
    return "heuristic_v15_pvs";
}

void HeuristicSearcherV15Pvs::clear_tt() {
    tt_.clear();
}

std::size_t HeuristicSearcherV15Pvs::tt_entry_count() const {
    return tt_.entry_count();
}

void HeuristicSearcherV15Pvs::clear_tt_stats() {
    tt_.clear_stats();
}

const RangeTranspositionTableStats& HeuristicSearcherV15Pvs::tt_stats() const {
    return tt_.stats();
}
