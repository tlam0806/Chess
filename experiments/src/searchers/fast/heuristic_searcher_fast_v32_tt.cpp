...
int HeuristicSearcherFastV32::score_to_tt(int score, int ply) const {
...
HeuristicSearcherFastV32::TTProbeResult HeuristicSearcherFastV32::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherFastV32::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void HeuristicSearcherFastV32::update_best_range(
...
void HeuristicSearcherFastV32::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
