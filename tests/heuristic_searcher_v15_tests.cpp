...
void assert_v15_matches_v14(Position pos, int max_depth) {
    for (int depth = 0; depth <= max_depth; ++depth) {
        HeuristicSearcherV14 v14(4);
        HeuristicSearcherV15 v15(4);
        const SearchResult a = v14.search_best_move(pos, depth);
        const SearchResult b = v15.search_best_move(pos, depth);
...
void assert_v15_matches_v14_from_fen(std::string_view fen, int max_depth) {
    Position pos;
    assert(pos.set_fen(fen));
    assert_v15_matches_v14(pos, max_depth);
}

void stress_v15_matches_v14(std::uint32_t seed, int samples, int max_random_plies, int max_depth) {
    ...
    for (int sample = 0; sample < samples; ++sample) {
        ...
        for (int depth = 0; depth <= max_depth; ++depth) {
            HeuristicSearcherV14 v14(8);
            HeuristicSearcherV15 v15(8);
            v14.clear_tt();
            v15.clear_tt();

            const SearchResult a = v14.search_best_move(pos, depth);
            const SearchResult b = v15.search_best_move(pos, depth);
            if (a.score != b.score || a.best_move != b.best_move) {
                std::cerr
                    << "v15 mismatch"
                    << " seed=" << seed
                    << " sample=" << sample
                    ...
                    << " v14_score=" << a.score
                    << " v15_score=" << b.score
                    << " v14_move=" << move_to_string(a.best_move)
                    << " v15_move=" << move_to_string(b.best_move)
                    << " fen=\"" << test_fen(pos) << "\""
                    << '\n';
                std::abort();
            }
...
int main() {
...

    {
        Position pos;
        pos.set_startpos();
        assert_v15_matches_v14(pos, 3);
    }

    assert_v15_matches_v14_from_fen(
        "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1",
        3);
    ...
    stress_v15_matches_v14(20260616, 250, 80, 4);
    stress_v15_matches_v14(20260617, 40, 100, 5);
...

    {
        ...
        const SearchResult a = v14.search_best_move(pos, limits);
        const SearchResult b = v15.search_best_move(pos, limits);
        assert(a.score == b.score);
        assert(a.best_move == b.best_move);
    }
}
