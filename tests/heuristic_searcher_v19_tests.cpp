...
void assert_v19_search_is_stable(Position pos, int max_depth) {
    HeuristicSearcherV19 v19(1);

    for (int depth = 0; depth <= max_depth; ++depth) {
        v19.clear_tt();
        const SearchResult cold = v19.search_best_move(pos, depth);
        const SearchResult warm = v19.search_best_move(pos, depth);
...
void assert_v19_search_is_stable_from_fen(std::string_view fen, int max_depth) {
    Position pos;
    assert(pos.set_fen(fen));
    assert_v19_search_is_stable(pos, max_depth);
}

void stress_v19_search_is_stable(std::uint32_t seed, int samples, int max_random_plies, int max_depth) {
...
int main() {
    {
        Position pos;
        pos.set_startpos();
        assert_v19_search_is_stable(pos, 3);
    }

    assert_v19_search_is_stable_from_fen(
...

    {
        Position pos;
        pos.set_startpos();
        assert_v19_search_is_stable(pos, 4);
    }
    ...
    stress_v19_search_is_stable(20260616, 250, 80, 4);
    stress_v19_search_is_stable(20260617, 40, 100, 5);

    {
        ...
        HeuristicSearcherV19 v19(1);
        const SearchResult result = v19.search_best_move(pos, SearchLimits{
            .max_depth = 64,
...

    {
        ...
        HeuristicSearcherV19 v19(4);
        const SearchLimits limits{
            .max_depth = 5,
            .move_time = std::chrono::milliseconds{0}
        };
        const SearchResult result = v19.search_best_move(pos, limits);
