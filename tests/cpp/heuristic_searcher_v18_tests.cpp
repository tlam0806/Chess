...
void assert_v18_search_is_stable(Position pos, int max_depth) {
    HeuristicSearcherV18 v18(1);

    for (int depth = 0; depth <= max_depth; ++depth) {
        v18.clear_tt();
        const SearchResult cold = v18.search_best_move(pos, depth);
        const SearchResult warm = v18.search_best_move(pos, depth);
...
void assert_v18_search_is_stable_from_fen(std::string_view fen, int max_depth) {
    Position pos;
    assert(pos.set_fen(fen));
    assert_v18_search_is_stable(pos, max_depth);
}

void stress_v18_search_is_stable(std::uint32_t seed, int samples, int max_random_plies, int max_depth) {
...
int main() {
    {
        Position pos;
        pos.set_startpos();
        assert_v18_search_is_stable(pos, 3);
    }

    assert_v18_search_is_stable_from_fen(
...

    {
        Position pos;
        pos.set_startpos();
        assert_v18_search_is_stable(pos, 4);
    }
...

    {
        ...
        HeuristicSearcherV18 v18(1);
        const SearchResult result = v18.search_best_move(pos, 1);
    ...
    stress_v18_search_is_stable(20260616, 250, 80, 4);
    stress_v18_search_is_stable(20260617, 40, 100, 5);

    {
        ...
        HeuristicSearcherV18 v18(1);
        const SearchResult result = v18.search_best_move(pos, SearchLimits{
            .max_depth = 64,
...

    {
        ...
        HeuristicSearcherV18 v18(4);
        const SearchLimits limits{
            .max_depth = 5,
            .move_time = std::chrono::milliseconds{0}
        };
        const SearchResult result = v18.search_best_move(pos, limits);
