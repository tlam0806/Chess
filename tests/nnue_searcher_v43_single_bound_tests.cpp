...
int main(int argc, char** argv) {
    ...
    const auto& production_selective = v43.selective_config();
    assert(production_selective.enable_lmr);
    assert(production_selective.lmr_base == 0.45);
    assert(production_selective.lmr_divisor == 2.9);
    assert(production_selective.lmr_min_depth == 3);
    assert(production_selective.lmr_min_move_index == 6);
    assert(production_selective.enable_null_move);
    assert(production_selective.null_move_min_depth == 2);
    assert(production_selective.null_move_reduction == 4);
    assert(production_selective.enable_reverse_futility);
    assert(production_selective.reverse_futility_max_depth == 4);
    assert(production_selective.reverse_futility_base_margin == 50);
    assert(production_selective.reverse_futility_margin_per_depth == 100);
    assert(production_selective.enable_late_move_pruning);
    assert(production_selective.late_move_pruning_max_depth == 3);
    assert(production_selective.late_move_pruning_base == 4);
    assert(production_selective.late_move_pruning_depth_multiplier == 1);
    assert(production_selective.enable_qsearch_see_pruning);
    assert(production_selective.qsearch_see_threshold == -25);
    assert(!production_selective.enable_main_search_see_pruning);
    const auto& production_aspiration = v43.aspiration_config();
    assert(production_aspiration.enabled);
    assert(production_aspiration.min_depth == 2);
    assert(production_aspiration.delta_base_cp == 68);
    assert(production_aspiration.delta_divisor == 33'700);
    assert(production_aspiration.expansion_factor_per_mille == 2'290);
    assert(production_aspiration.max_fail_high_reductions == 1);
    assert(production_aspiration.mean_score_new_weight_per_mille == 370);
    assert(production_aspiration.max_researches == 6);
    assert(production_aspiration.mean_score_clamp_cp == 1'500);
