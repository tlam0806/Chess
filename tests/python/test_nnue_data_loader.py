...
from tools.train.train_nnue_architecture import (
    CompactSplitDataset,
    JsonlSplitDataset,
    cp_huber_loss,
    make_loader,
...
class NnueDataLoaderTests(unittest.TestCase):
    ...
    def test_shards_are_partitioned_without_worker_overlap(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for worker_id in range(4):
                ...
                with patch("tools.train.train_nnue_architecture.get_worker_info", return_value=worker):
    ...
    def test_unshuffled_shards_are_opened_by_only_one_worker(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for worker_id in range(4):
                ...
                worker = SimpleNamespace(id=worker_id, num_workers=4)
                with (
                    patch("tools.train.train_nnue_architecture.get_worker_info", return_value=worker),
                    patch("tools.train.train_nnue_architecture.iter_compact_samples", fake_samples),
                    patch(
                        "tools.train.train_nnue_architecture.canonical_architecture_input",
    ...
    def test_truncated_compact_record_is_rejected_by_reader(self) -> None:
        from chess_nnue.compact_board_data import HEADER, iter_compact_samples
