...
from tools.tune import tune_nnue_v43_aspiration as tuner
...
def evaluator_result(config: tuner.Config, narrow: int = 1) -> dict:
    value = {
        ...
        "candidate_time_ms": 0,
        "candidate_mean_depth": 6.0,
        "candidate_completed_depth_count": 10,
        ...
        **tuner.CANDIDATE_ENGINE_PROFILE,
    }
...
def test_canonical_hash_covers_all_nine_fields() -> None:
    ...
    assert tuner.replace(
        baseline, enabled=False).label.startswith("v43-legacy-fixed50-")
...
def test_candidate_engine_args_pin_v43_single_bound_tt_profile() -> None:
    args = tuner.candidate_engine_evaluator_args()
    assert args == [
        "--tt-mb", "64",
        "--tt-bucket-size", "4",
    ...
    assert tuner.CANDIDATE_ENGINE_PROFILE == {
...
def test_cli_rejects_time_mode_and_mutable_guards(monkeypatch) -> None:
    base = [
        "tune_nnue_v43_aspiration.py",
        "--binary", "evaluator",
        "--dataset-dir", "dataset",
        "--model", "model",
        "--run-dir", "run",
    ...
    parsed = tuner.parse_args()
    assert parsed.candidate_time_ms == 0
    assert parsed.max_researches == tuner.FROZEN_MAX_RESEARCHES
    assert parsed.mean_score_clamp_cp == tuner.FROZEN_MEAN_SCORE_CLAMP_CP

    monkeypatch.setattr(sys, "argv", [*base, "--candidate-time-ms", "1"])
    with pytest.raises(SystemExit):
        tuner.parse_args()
    monkeypatch.setattr(sys, "argv", [*base, "--max-researches", "7"])
    with pytest.raises(SystemExit):
        tuner.parse_args()
    monkeypatch.setattr(
        sys, "argv", [*base, "--mean-score-clamp-cp", "1499"])
    with pytest.raises(SystemExit):
        tuner.parse_args()
...
def test_selfplay_args_pin_both_profiles_and_twofold_policy() -> None:
    ...
    assert payload["reuse_stale_tt_scores_for_both"] is False
    assert payload["reuse_deeper_tt_scores_for_both"] is False
    assert payload["control_name"] == "v43_legacy_fixed50_control"
...
def test_global_plan_contains_anchors_and_exact_requested_count() -> None:
    ...
    assert "anchor:v43-legacy-fixed50" in origins
    assert "anchor:v43-default-seed" in origins
    assert {f"anchor:prior-v42-{name}" for name in ("fast", "balanced", "safe")} <= origins
...
def test_named_profiles_prefer_baseline_inclusive_selection_frontier() -> None:
    diagnostic_fast = result_record(
        tuner.replace(tuner.Config(), delta_base_cp=11), 0.50, 0.020)
    surviving_balanced = result_record(
        tuner.replace(tuner.Config(), delta_base_cp=22), 0.80, 0.002)
    surviving_safe = result_record(
        tuner.replace(tuner.Config(), delta_base_cp=33), 0.90, 0.001)
    enabled = [diagnostic_fast, surviving_balanced, surviving_safe]
    on_full = [surviving_balanced, surviving_safe]

    profiles, source = tuner.choose_named_profiles(enabled, on_full)
    assert source == "selection_baseline_inclusive_frontier"
    assert {entry["config_hash"] for _, entry in profiles} <= {
        surviving_balanced["config_hash"], surviving_safe["config_hash"]}
    assert diagnostic_fast["config_hash"] not in {
        entry["config_hash"] for _, entry in profiles}


def test_named_profiles_without_full_frontier_survivor_are_diagnostic() -> None:
    fast = result_record(tuner.Config(), 0.50, 0.020)
    safe = result_record(
        tuner.replace(tuner.Config(), delta_base_cp=40), 0.90, 0.001)
    profiles, source = tuner.choose_named_profiles([fast, safe], [])
    assert len(profiles) == 3
    assert source == "selection_adaptive_only_diagnostic"

    baseline = result_record(
        tuner.replace(tuner.Config(), enabled=False), 1.0, 0.001)
    baseline["result"]["candidate_nodes"] = 1_000
    profiles[0][1]["result"]["candidate_nodes"] = 500
    payload = tuner.annotate_profile_frontiers(
        profiles[0][1], baseline, set(), {profiles[0][1]["config_hash"]},
        source)
    assert payload["selection_on_full_frontier"] is False
    assert payload["holdout_on_full_frontier"] is True
    assert payload["offline_profile_status"] == (
        "diagnostic_only_not_on_selection_frontier")
    assert tuner.summarize_offline_promotion(
        [False, False, False], [True, True, True]) == (
        "diagnostic_only_no_selection_frontier_promotion")


def test_holdout_only_rejects_preselected_profiles() -> None:
    assert tuner.summarize_offline_promotion(
        [True, True, True], [True, True, True]
    ) == "holdout_confirmed_offline_promotion"
    assert tuner.summarize_offline_promotion(
        [True, True, True], [True, False, False]
    ) == "holdout_partially_rejected_offline_promotion"
    assert tuner.summarize_offline_promotion(
        [True, True, True], [False, False, False]
    ) == "holdout_rejected_offline_promotion"
...
def test_result_validation_requires_effective_config_and_real_aspiration() -> None:
    ...
    range_conflict = evaluator_result(config)
    range_conflict["aspiration_range_conflict_fallbacks"] = 1
    with pytest.raises(RuntimeError, match="range conflict"):
        tuner.validate_result(range_conflict, config, 6, 0, control_cache)

    unresolved = evaluator_result(config)
    unresolved["aspiration_unresolved_ranges"] = 1
    with pytest.raises(RuntimeError, match="unresolved output"):
        tuner.validate_result(unresolved, config, 6, 0, control_cache)

    wrong_depth = evaluator_result(config)
    wrong_depth["candidate_mean_depth"] = 5.0
    with pytest.raises(RuntimeError, match="wrong depth"):
        tuner.validate_result(wrong_depth, config, 6, 0, control_cache)

    inconsistent_depth_ratio = evaluator_result(config)
    inconsistent_depth_ratio.update({
        "aspiration_accepted_narrow_nominal_depth_sum": 10,
        "aspiration_accepted_narrow_search_depth_sum": 8,
        "aspiration_accepted_depth_ratio": 0.9,
    })
    with pytest.raises(RuntimeError, match="disagrees with depth sums"):
        tuner.validate_result(
            inconsistent_depth_ratio, config, 6, 0, control_cache)
...
@pytest.mark.parametrize(
    "field,value",
    [
        ("candidate_engine", "NnueSearcherV42"),
        ("searcher_version", 42),
        ("tt_mb", 128),
        ("tt_bucket_size", 8),
        ("tt_layout", "range"),
        ("reuse_stale_tt_scores", True),
        ("reuse_deeper_tt_scores", True),
        ("tt_depth_policy", "AtLeast"),
        ("tt_generation_policy", "Any"),
    ],
)
def test_result_validation_rejects_every_v43_profile_drift(field, value) -> None:
    config = tuner.Config()
    result = evaluator_result(config)
    result[field] = value
    with pytest.raises(RuntimeError, match=f"candidate profile {field}"):
        tuner.validate_result(
...
def test_relative_profile_uses_candidate_nodes_not_v36_ratio() -> None:
    baseline = result_record(tuner.replace(tuner.Config(), enabled=False), 0.1, 0.002)
    candidate = result_record(tuner.Config(), 0.05, 0.003)
    baseline["result"]["candidate_nodes"] = 1_000
    candidate["result"]["candidate_nodes"] = 600
    profile = tuner.relative_profile(candidate, baseline)
    assert profile["node_ratio_vs_v43_legacy_fixed50"] == pytest.approx(0.6)
    assert profile["wdl_loss_delta_vs_v43_legacy_fixed50"] == pytest.approx(0.001)
...
def test_sealed_dataset_manifest_is_required_and_cross_checked(tmp_path) -> None:
    ...
    manifest = {
        "schema_version": 2,
        "format": tuner.SEALED_DATASET_FORMAT,
        "selection_policy": {
            "split_unit": "game_id",
...
def test_control_cache_identity_pins_every_semantic_input(tmp_path) -> None:
    ...
    runtime = baseline["identity"]["evaluator_runtime_profile"]
    assert runtime["candidate_engine"] == tuner.CANDIDATE_ENGINE_PROFILE
    assert runtime["selective_config"] == tuner.PRODUCTION_SELECTIVE_CONFIG
    assert runtime["twofold_search_draw_enabled"] is True
    assert runtime["adaptive_disabled_baseline"] == {
