...
class TrainValueTests(unittest.TestCase):
    def test_train_one_epoch_and_save_checkpoint(self):
        ...
        from chess_nnue.train_value import (
            DEFAULT_TARGET_CLIP,
        ...
        from chess_nnue.training_targets import STOCKFISH_VALUE_NONE
        from chess_nnue.value_net import AUX_FEATURE_COUNT, ChessValueNet
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            samples = [
                ...
                {"features": [30], "aux": [0] * AUX_FEATURE_COUNT, "target": STOCKFISH_VALUE_NONE},
            ]
            ...
            self.assertEqual(normalize_target_cp(2500.0, 1000.0, DEFAULT_TARGET_CLIP), 2.5)
            self.assertEqual(normalize_target_cp(-1500.0, 1000.0, DEFAULT_TARGET_CLIP), -1.5)
            ...
            _, _, _, unclipped_targets = collate_sparse_value_batch(
                [
                    {"features": [1], "aux": [0] * AUX_FEATURE_COUNT, "target": 2500.0},
                    {"features": [2], "aux": [0] * AUX_FEATURE_COUNT, "target": -1500.0},
                ],
            ...
            self.assertEqual(unclipped_targets.tolist(), [2.5, -1.5])

            with self.assertRaisesRegex(ValueError, "VALUE_NONE"):
                collate_sparse_value_batch(
                    [
                        {
                            "features": [1],
                            "aux": [0] * AUX_FEATURE_COUNT,
                            "target": STOCKFISH_VALUE_NONE,
                        }
                    ]
                )
