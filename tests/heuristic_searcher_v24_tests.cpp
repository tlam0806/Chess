...
void assert_v24_search_is_stable(Position pos, int max_depth) {
    HeuristicSearcherV24 v24(1);

    for (int depth = 0; depth <= max_depth; ++depth) {
        v24.clear_tt();
        const SearchResult cold = v24.search_best_move(pos, depth);
        const SearchResult warm = v24.search_best_move(pos, depth);
...
int main() {
...

    {
        ...
        for (int depth = 1; depth <= 3; ++depth) {
            v24.clear_tt();
            const SearchResult result = v24.search_best_move(pos, depth);
...

    {
        ...
        HeuristicSearcherV24 v24(1);
        const SearchResult child_result = v24.search_best_move(stalemate_child, 1);
        assert(child_result.score == 0);
        assert(child_result.nodes == 1);

        const SearchResult parent_depth_2 = v24.search_best_move(pos, 2);
...

    {
        ...
        HeuristicSearcherV24 v24(1);
        const SearchResult qsearch_result = v24.search_best_move(pos, 0);
...

    {
        ...
        HeuristicSearcherV24 v24(1);
        const SearchResult result = v24.search_best_move(pos, SearchLimits{
            .max_depth = 64,
...

    {
        ...
        HeuristicSearcherV24 v24(4);
        const SearchLimits limits{
            .max_depth = 5,
            .move_time = std::chrono::milliseconds{0}
        };
        const SearchResult result = v24.search_best_move(pos, limits);
