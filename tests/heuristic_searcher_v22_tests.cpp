...
void assert_v22_search_is_stable(Position pos, int max_depth) {
    HeuristicSearcherV22 v22(1);

    for (int depth = 0; depth <= max_depth; ++depth) {
        v22.clear_tt();
        const SearchResult cold = v22.search_best_move(pos, depth);
        const SearchResult warm = v22.search_best_move(pos, depth);
...
void assert_v22_search_is_stable_from_fen(std::string_view fen, int max_depth) {
    Position pos;
    assert(pos.set_fen(fen));
    assert_v22_search_is_stable(pos, max_depth);
}

void stress_v22_search_is_stable(std::uint32_t seed, int samples, int max_random_plies, int max_depth) {
...
int main() {
    {
        Position pos;
        pos.set_startpos();
        assert_v22_search_is_stable(pos, 3);
    }

    assert_v22_search_is_stable_from_fen(
...

    {
        Position pos;
        pos.set_startpos();
        assert_v22_search_is_stable(pos, 4);
    }
...

    {
        ...
        for (int depth = 1; depth <= 3; ++depth) {
            v22.clear_tt();
            const SearchResult result = v22.search_best_move(pos, depth);
...

    {
        ...
        HeuristicSearcherV22 v22(1);
        const SearchResult child_result = v22.search_best_move(stalemate_child, 1);
        assert(child_result.score == 0);
        assert(child_result.nodes == 1);

        const SearchResult parent_depth_2 = v22.search_best_move(pos, 2);
...

    {
        ...
        HeuristicSearcherV22 v22(1);
        const SearchResult qsearch_result = v22.search_best_move(pos, 0);
    ...
    stress_v22_search_is_stable(20260616, 250, 80, 4);
    stress_v22_search_is_stable(20260617, 40, 100, 5);

    {
        ...
        HeuristicSearcherV22 v22(1);
        const SearchResult result = v22.search_best_move(pos, SearchLimits{
            .max_depth = 64,
...

    {
        ...
        HeuristicSearcherV22 v22(4);
        const SearchLimits limits{
            .max_depth = 5,
            .move_time = std::chrono::milliseconds{0}
        };
        const SearchResult result = v22.search_best_move(pos, limits);
