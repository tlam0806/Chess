...
void assert_v24_search_is_stable(Position pos, int max_depth) {
    HeuristicSearcherV25 v25(1);

    for (int depth = 0; depth <= max_depth; ++depth) {
        v25.clear_tt();
        const SearchResult cold = v25.search_best_move(pos, depth);
        const SearchResult warm = v25.search_best_move(pos, depth);
...
void assert_v24_search_is_stable_from_fen(std::string_view fen, int max_depth) {
    Position pos;
    assert(pos.set_fen(fen));
    assert_v24_search_is_stable(pos, max_depth);
}

void stress_v24_search_is_stable(std::uint32_t seed, int samples, int max_random_plies, int max_depth) {
...
int main() {
    {
        Position pos;
        pos.set_startpos();
        assert_v24_search_is_stable(pos, 3);
    }
...

    {
        Position pos;
        pos.set_startpos();
        assert_v24_search_is_stable(pos, 4);
    }
...

    {
        ...
        for (int depth = 1; depth <= 3; ++depth) {
            v25.clear_tt();
            const SearchResult result = v25.search_best_move(pos, depth);
...

    {
        ...
        HeuristicSearcherV25 v25(1);
        const SearchResult child_result = v25.search_best_move(stalemate_child, 1);
        assert(child_result.score == 0);
        assert(child_result.nodes == 1);

        const SearchResult parent_depth_2 = v25.search_best_move(pos, 2);
...

    {
        ...
        HeuristicSearcherV25 v25(1);
        const SearchResult qsearch_result = v25.search_best_move(pos, 0);
    ...
    stress_v24_search_is_stable(20260616, 250, 80, 4);
    stress_v24_search_is_stable(20260617, 40, 100, 5);

    {
        ...
        HeuristicSearcherV25 v25(1);
        const SearchResult result = v25.search_best_move(pos, SearchLimits{
            .max_depth = 64,
...

    {
        ...
        HeuristicSearcherV25 v25(4);
        const SearchLimits limits{
            .max_depth = 5,
            .move_time = std::chrono::milliseconds{0}
        };
        const SearchResult result = v25.search_best_move(pos, limits);
