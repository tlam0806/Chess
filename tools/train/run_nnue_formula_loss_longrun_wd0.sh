...
run_config() {
  ...
  local command=("$python_bin" tools/train/train_quantized_nnue_architecture.py \
    --arch F2 \
...
if [[ "${RESUME_SELECTION_ONLY:-0}" != "1" ]]; then
  ...
  mapfile_output="$($python_bin tools/analyze/select_nnue_longrun_candidates.py \
...
"$python_bin" tools/analyze/summarize_nnue_loss_quantization_ablation.py \
