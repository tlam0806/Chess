...
TRAINER = REPO_ROOT / "tools" / "train" / "train_quantized_nnue_architecture.py"


def main() -> int:
    parser = argparse.ArgumentParser(description="Run clip180/div128 ScReLU high hidden-scale ablation")
    ...
    configs = [
        {"label": "c180_d128_hs16x32", "clip": 180, "divisor": 128, "hs": [16, 32]},
        {"label": "c180_d128_hs32x16", "clip": 180, "divisor": 128, "hs": [32, 16]},
        {"label": "c180_d128_hs32x32", "clip": 180, "divisor": 128, "hs": [32, 32]},
    ]
    ...
    manifest = {
        "tag": args.tag,
        "comparison": "clip180/div128 ScReLU high hidden-scale extension",
        "data": str(args.data),
