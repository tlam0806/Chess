...
from chess_nnue.train_value import collate_sparse_value_batch
...
def load_model(checkpoint_path: Path) -> tuple[ChessNnueValueNet, float, float]:
...
def export_binary(checkpoint_path: Path, output_path: Path) -> None:
    ...
    with output_path.open("wb") as file:
        ...
        write_tensor(file, model.feature_weights.weight)
        write_tensor(file, model.aux_projection.weight)
        write_tensor(file, model.accumulator_bias)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export ChessNnueValueNet weights")
