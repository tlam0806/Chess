...
.venv/bin/python tools/data/mix_jsonl_dataset.py \
  --output "${MIX}" \
  --seed 63014 \
  --source "${RAW}:40000" \
...
.venv/bin/python -m chess_nnue.train_nnue_value \
  --train "${MIX}" \
...
.venv/bin/python tools/analyze/evaluate_nnue_value.py \
...
.venv/bin/python tools/train/export_nnue_value_net.py \
  --checkpoint "${MODEL_OUT}" \
