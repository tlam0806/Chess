...
int main(int argc, char** argv) {
    ...
    for (std::size_t i = 0; i < positions.size(); ++i) {
        ...
        const Row r33 = run_one(v33, positions[i], depth, iterative);
        const Row r32 = run_one(v32, positions[i], depth, iterative);

        v33_us += r33.us;
        v32_us += r32.us;
        v33_nodes += r33.result.nodes;
        v32_nodes += r32.result.nodes;
        if (r33.result.score != r32.result.score) {
            ++score_mismatches;
        }
        if (r33.result.best_move != r32.result.best_move) {
            ++move_mismatches;
        }

        std::cout << "sample=" << i
                  << " v33_score=" << r33.result.score
                  << " v32_score=" << r32.result.score
                  << " v33_best=" << chess::move_to_string(r33.result.best_move)
    ...
    std::cout << "summary"
              ...
              << " v32_vs_v33_time=" << (v33_us == 0 ? 0.0 : static_cast<double>(v32_us) / v33_us)
