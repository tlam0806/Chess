...
"$python_bin" tools/audit_nnue_float_quantized.py \
...
"$python_bin" tools/export_quantized_candidate_fixture.py \
  --checkpoint "$checkpoint" \
  --data "$data" \
  --data-format cbin \
  --device "$device" \
...
"$cxx" -std=c++20 -O2 -Wall -Wextra -Wpedantic \
  tools/evaluate_quantized_cpp_fixture.cpp \
...
"$python_bin" tools/summarize_nnue_inference_audit.py \
