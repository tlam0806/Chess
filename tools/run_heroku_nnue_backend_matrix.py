...
try:
    ...
    from tools import run_heroku_uci_platform_benchmark as platform_runner
except ModuleNotFoundError:  # Direct execution adds tools/, not the repo root.
    import compare_nnue_backend_benchmarks as comparator
    import run_heroku_uci_platform_benchmark as platform_runner
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--local-remote-runner",
        type=Path,
        default=repo_root / "tools/run_nnue_backend_matrix_remote.py",
    )
    parser.add_argument(
        "--local-platform-runner",
        type=Path,
        default=repo_root / "tools/run_heroku_uci_platform_benchmark.py",
    )
    parser.add_argument(
        "--local-comparator",
        type=Path,
        default=repo_root / "tools/compare_nnue_backend_benchmarks.py",
    )
    parser.add_argument(
        "--local-spec",
        type=Path,
    ...
    parser.add_argument(
        "--remote-harness", default=f"{remote_root}/tools/benchmark_uci_platform.py"
    )
    parser.add_argument(
        "--remote-matrix-runner",
        default=f"{remote_root}/tools/run_nnue_backend_matrix_remote.py",
    )
