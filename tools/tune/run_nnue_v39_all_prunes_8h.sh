...
RUN_DIR="${1:?usage: run_nnue_v39_all_prunes_8h.sh RUN_DIR}"
...
.venv/bin/python tools/tune/tune_nnue_v39_all_prunes.py \
  --binary build-release/evaluate_nnue_v39_selective \
  --dataset-dir "$DATASET_DIR" \
  --model "$MODEL" \
  --run-dir "$RUN_DIR" \
  --duration-sec 28800 \
  --mutation-duration-sec 10800 \
  --max-candidates 180 \
  --tune-size 1500 \
  --rotation-count 4 \
  --probe-size 600 \
  --time-selection-size 1200 \
  --audit-size 1000 \
  --tune-depth 6 \
  --selection-depth 7 \
  --audit-depth 8 \
