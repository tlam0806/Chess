...
RUN_DIR="${1:?usage: run_nnue_v42_aspiration_tune.sh RUN_DIR}"
if [[ "$RUN_DIR" != /* ]]; then
  RUN_DIR="$REPO_ROOT/$RUN_DIR"
fi

DATASET_DIR="${NNUE_V42_ASPIRATION_DATASET_DIR:-data/nnue_v42_aspiration_lichess_20260830}"
MODEL="${NNUE_V42_ASPIRATION_MODEL:-models/quantized_scale_grid/old_score_huber200_lr_sweep_then_5ep_20260724_142758/best/phase_quantized_nnue.bin}"
BUILD_DIR="${NNUE_V42_ASPIRATION_BUILD_DIR:-build-v42-aspiration-release}"
BUILD_LOCK_FILE="${NNUE_V42_ASPIRATION_BUILD_LOCK_FILE:-${BUILD_DIR}.nnue_v42_aspiration.lock}"
RUN_BINARY_DIR="$RUN_DIR/bin"
RUN_INPUT_DIR="$RUN_DIR/inputs"
EVALUATOR_BINARY="$RUN_BINARY_DIR/evaluate_nnue_v42_selective"
GAUNTLET_BINARY="$RUN_BINARY_DIR/nnue_v42_time_gauntlet"
TUNER_SOURCE="${NNUE_V42_ASPIRATION_TUNER_SOURCE:-tools/tune/tune_nnue_v42_aspiration.py}"
RUN_TUNER="$RUN_BINARY_DIR/tune_nnue_v42_aspiration.py"
RUN_MODEL="$RUN_INPUT_DIR/phase_quantized_nnue.bin"
PYTHON="${NNUE_V42_ASPIRATION_PYTHON:-.venv/bin/python}"
# This is the cumulative elapsed ceiling for the run, not a fresh allowance
# per invocation.  Resume a PAUSED run by setting a strictly larger value.
DURATION_SEC="${NNUE_V42_ASPIRATION_CUMULATIVE_DURATION_SEC:-${NNUE_V42_ASPIRATION_DURATION_SEC:-28800}}"
WORKERS="${NNUE_V42_ASPIRATION_WORKERS:-4}"
BUILD_JOBS="${NNUE_V42_ASPIRATION_BUILD_JOBS:-8}"
SEED="${NNUE_V42_ASPIRATION_SEED:-20260829}"
CAFFEINATE_PID=""
SHLOCK_HELD=0
TUNER_PID=""
...
if [[ -s "$RUN_DIR/runner.pid" ]]; then
  ...
  if [[ "$prior_pid" =~ ^[0-9]+$ ]] && kill -0 "$prior_pid" 2>/dev/null; then
    echo "another aspiration tune runner is active: pid=$prior_pid" >&2
    exit 1
  fi
fi
...
' "$DATASET_DIR/manifest.json"; then
  echo "dataset manifest is not sealed V42 Lichess schema v2" >&2
  exit 1
fi
...
build_targets() {
  ...
  cmake --build "$BUILD_DIR" \
    --clean-first \
    --target evaluate_nnue_v42_selective nnue_v42_time_gauntlet \
    -j "$BUILD_JOBS"
  snapshot_file \
    "$BUILD_DIR/evaluate_nnue_v42_selective" "$EVALUATOR_BINARY" 755
  snapshot_file \
    "$BUILD_DIR/nnue_v42_time_gauntlet" "$GAUNTLET_BINARY" 755
