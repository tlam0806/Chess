...
stream_sources() {
  ...
  for name in "${remote_files[@]}"; do
    ...
    tools/data/download_zstd_with_resume.sh \
      "$base_url/$name" \
...
stream_sources \
  ...
  | /usr/bin/nice -n 15 .venv/bin/python tools/data/build_stratified_cp_corpus.py \
      --source - \
...
.venv/bin/python tools/data/stream_cbin_shards.py --input "$labeled_dir" \
