...
struct MoveOrderingTotals {
    ...
    std::array<std::uint64_t, chess::HeuristicSearcherV16::MoveOrderingCategoryCount>
        missed_predicted_categories{};
    std::array<std::uint64_t, chess::HeuristicSearcherV16::MoveOrderingCategoryCount>
        missed_best_categories{};
};
...
Options parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--input") {
            ...
            std::cout
                << "Usage: benchmark_v10_v16_depth_sweep [--input path]\n"
                << "                                      [--samples N]\n"
                << "                                      [--min-depth D]\n"
                << "                                      [--max-depth D]\n"
                << "                                      [--seed N]\n"
...
void add_ordering_stats(
    MoveOrderingTotals& totals,
    const chess::HeuristicSearcherV16::MoveOrderingStats& stats
) {
    ...
    for (std::size_t i = 0; i < chess::HeuristicSearcherV16::MoveOrderingCategoryCount; ++i) {
...
std::string_view category_name(std::size_t index) {
    using Category = chess::HeuristicSearcherV16::MoveOrderingCategory;
    switch (static_cast<Category>(index)) {
    case Category::TtLower: return "tt_lower";
    case Category::TtUpper: return "tt_upper";
    case Category::Promotion: return "promotion";
    case Category::GoodCapture: return "good_capture";
    case Category::BadCapture: return "bad_capture";
    case Category::Check: return "check";
    case Category::Killer1: return "killer1";
    case Category::Killer2: return "killer2";
    case Category::Quiet: return "quiet";
...
int main(int argc, char** argv) {
    try {
        ...
        for (int depth = options.min_depth; depth <= options.max_depth; ++depth) {
            ...
            TtTotals v10_tt;
            TtTotals v16_tt;
            MoveOrderingTotals v16_ordering;
            ...
            for (const Sample& sample : samples) {
                ...
                const chess::SearchResult v16_result = search_iterative(v16, sample.pos, depth);
                ...
                add_tt_stats(v16_tt, v16.tt_stats());
                add_ordering_stats(v16_ordering, v16.move_ordering_stats());
                if (v10_result.score != v16_result.score) {
                    ++v10_totals.score_mismatches;
                    ++v16_totals.score_mismatches;
                }
                if (v10_result.best_move != v16_result.best_move) {
            ...
            if (v10_totals.nodes != 0) {
                std::cout << " v16_vs_v10_nodes="
                          << (static_cast<double>(v16_totals.nodes) / static_cast<double>(v10_totals.nodes));
            }
            if (v10_totals.time_us != 0) {
                std::cout << " v16_vs_v10_time="
                          << (static_cast<double>(v16_totals.time_us) / static_cast<double>(v10_totals.time_us));
            }
            std::cout << " score_mismatches=" << v10_totals.score_mismatches
                      ...
                      << " v16_tt_probes=" << v16_tt.probes
                      << " v16_tt_key_hits=" << v16_tt.key_hits
                      << " v16_tt_key_hit_rate=" << ratio(v16_tt.key_hits, v16_tt.probes)
                      << " v16_tt_score_hits=" << v16_tt.score_hits
                      << " v16_tt_score_hit_rate=" << ratio(v16_tt.score_hits, v16_tt.probes)
                      << " v16_tt_score_returns=" << v16_tt.score_returns
                      << " v16_tt_move_hint_hits=" << v16_tt.move_hint_hits
                      << " v16_tt_move_hint_rate=" << ratio(v16_tt.move_hint_hits, v16_tt.probes)
                      << " v16_tt_index_collisions=" << v16_tt.index_collisions
                      << " v16_tt_probe_collision_rate=" << ratio(v16_tt.index_collisions, v16_tt.probes)
                      << " v16_tt_depth_misses=" << v16_tt.depth_misses
                      << " v16_tt_stores=" << v16_tt.stores
                      << " v16_tt_replacement_collisions=" << v16_tt.replacement_collisions
                      << " v16_tt_store_collision_rate=" << ratio(v16_tt.replacement_collisions, v16_tt.stores)
                      << " v16_order_nodes=" << v16_ordering.searched_nodes
                      << " v16_best_index0=" << v16_ordering.best_index_zero
                      << " v16_best_index0_rate=" << ratio(v16_ordering.best_index_zero, v16_ordering.searched_nodes)
                      << " v16_avg_best_index=" << ratio(v16_ordering.best_index_sum, v16_ordering.searched_nodes)
                      << " v16_cutoff_nodes=" << v16_ordering.cutoff_nodes
                      << " v16_cutoff_index0=" << v16_ordering.cutoff_index_zero
                      << " v16_cutoff_index0_rate=" << ratio(v16_ordering.cutoff_index_zero, v16_ordering.cutoff_nodes)
                      << " v16_avg_cutoff_index=" << ratio(v16_ordering.cutoff_index_sum, v16_ordering.cutoff_nodes)
                      << " v16_missed_best_nodes=" << v16_ordering.missed_best_nodes;
            for (std::size_t i = 0; i < chess::HeuristicSearcherV16::MoveOrderingCategoryCount; ++i) {
                std::cout << " miss_pred_" << category_name(i) << "="
                          << v16_ordering.missed_predicted_categories[i];
            }
            for (std::size_t i = 0; i < chess::HeuristicSearcherV16::MoveOrderingCategoryCount; ++i) {
                std::cout << " miss_best_" << category_name(i) << "="
                          << v16_ordering.missed_best_categories[i];
            }
            std::cout << '\n' << std::flush;
        }
    } catch (const std::exception& error) {
        std::cerr << "benchmark_v10_v16_depth_sweep: " << error.what() << '\n';
        return 1;
    }
}
