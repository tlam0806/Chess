...
int HeuristicSearcherV29See::score_to_tt(int score, int ply) const {
...
HeuristicSearcherV29See::TTProbeResult HeuristicSearcherV29See::probe_tt(
    HashKey key,
    int depth,
    int& alpha,
    int& beta,
...
bool HeuristicSearcherV29See::should_store_tt(ScoreRange range) const {
    return EnableTt
        && (range.lower != range.upper || EnableTtExactStore);
}

void HeuristicSearcherV29See::update_best_range(
...
void HeuristicSearcherV29See::store_tt_if_needed(
    HashKey key,
    int depth,
    int ply,
    ScoreRange range,
