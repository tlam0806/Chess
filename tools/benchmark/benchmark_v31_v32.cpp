...
int main(int argc, char** argv) {
    ...
    std::uint64_t v31_us = 0;
    std::uint64_t v32_us = 0;
    std::uint64_t v31_nodes = 0;
    ...
    for (std::size_t i = 0; i < positions.size(); ++i) {
        ...
        chess::HeuristicSearcherV31 v31;
        const Row r31 = run_one(v31, positions[i], depth, iterative);
        const Row r32 = run_one(v32, positions[i], depth, iterative);

        v31_us += r31.us;
        v32_us += r32.us;
        v31_nodes += r31.result.nodes;
        v32_nodes += r32.result.nodes;
        if (r31.result.score != r32.result.score) {
            ++score_mismatches;
        }
        if (r31.result.best_move != r32.result.best_move) {
            ++move_mismatches;
        }

        std::cout << "sample=" << i
                  << " v31_score=" << r31.result.score
                  << " v32_score=" << r32.result.score
                  << " v31_best=" << chess::move_to_string(r31.result.best_move)
    ...
    std::cout << "summary"
              ...
              << " v31_us=" << v31_us
              << " v32_us=" << v32_us
              << " v31_vs_v32_time=" << (v32_us == 0 ? 0.0 : static_cast<double>(v31_us) / v32_us)
              << " v32_vs_v31_time=" << (v31_us == 0 ? 0.0 : static_cast<double>(v32_us) / v31_us)
