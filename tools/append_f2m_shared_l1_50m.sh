...
for required in .venv/bin/python tools/train_phase_component_nnue.py tools/export_phase_quantized_nnue.py tools/verify_phase_training_checkpoint.py tools/report_f2_f2m_50m.py; do
  if [[ ! -e "$required" ]]; then
...
.venv/bin/python tools/verify_phase_training_checkpoint.py --checkpoint "$OUTPUT_DIR/phase_component_best.pt" --arch F2M --phase-layout shared_first --expected-train-samples "$TRAIN_SAMPLES" > "$RUN_ROOT/f2m_shared_l1_checkpoint.json"
.venv/bin/python tools/export_phase_quantized_nnue.py --checkpoint "$OUTPUT_DIR/phase_component_best.pt" --output "$OUTPUT_DIR/phase_quantized_nnue.bin" --parity-data "$EVAL_DATA" --parity-output "$OUTPUT_DIR/phase_quantized_nnue_parity.tsv" --parity-samples 8192 > "$RUN_ROOT/f2m_shared_l1_export.log"
...
.venv/bin/python tools/report_f2_f2m_50m.py \
  --f2-dir "$F2_DIR" \
  --f2m-dir "$F2M_DIR" \
