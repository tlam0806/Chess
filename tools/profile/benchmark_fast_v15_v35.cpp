...
Options parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--depth") {
            ...
            std::cout
                << "Usage: benchmark_fast_v15_v35 [--depth N] [--tt-mb N]\n"
...
std::vector<chess::Position> make_positions(const Options& options) {
    ...
    positions.reserve(fens.size() + static_cast<std::size_t>(options.random_positions));
    ...
    for (int index = fixed_start; index < fixed_end; ++index) {
        const std::string_view fen = fens[static_cast<std::size_t>(index)];
        chess::Position pos;
        if (!pos.set_fen(fen)) {
...
int main(int argc, char** argv) {
    try {
        ...
        std::uint64_t v15_us = 0;
        std::uint64_t v35_us = 0;
        std::uint64_t v15_nodes = 0;
        std::uint64_t v35_nodes = 0;
        int score_mismatches = 0;
        int move_mismatches = 0;

        for (std::size_t i = 0; i < positions.size(); ++i) {
            chess::HeuristicSearcherV15Pvs v15(static_cast<std::size_t>(options.tt_mb));
            chess::HeuristicSearcherFastV35 v35(static_cast<std::size_t>(options.tt_mb));

            const Row r15 = run_one(v15, positions[i], options.depth);
            const Row r35 = run_one(v35, positions[i], options.depth);

            v15_us += r15.us;
            v35_us += r35.us;
            v15_nodes += r15.result.nodes;
            v35_nodes += r35.result.nodes;
            if (r15.result.score != r35.result.score) {
                ++score_mismatches;
            }
            if (r15.result.best_move != r35.result.best_move) {
                ++move_mismatches;
            }

            std::cout << "sample=" << i
                      << " v15_score=" << r15.result.score
                      << " v35_score=" << r35.result.score
                      << " v15_best=" << chess::move_to_string(r15.result.best_move)
        ...
        std::cout << "summary"
                  ...
                  << " v15_us=" << v15_us
                  << " v35_us=" << v35_us
                  << " v15_nps=" << nps(v15_nodes, v15_us)
                  << " v35_nps=" << nps(v35_nodes, v35_us)
                  << " v35_vs_v15_time=" << (v15_us == 0 ? 0.0 : static_cast<double>(v35_us) / v15_us)
                  << " v35_vs_v15_nodes=" << (v15_nodes == 0 ? 0.0 : static_cast<double>(v35_nodes) / v15_nodes)
