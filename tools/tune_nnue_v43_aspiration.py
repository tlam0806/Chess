#!/usr/bin/env python3
"""Resumable multi-fidelity tuner for V43 adaptive aspiration windows.
...
SEALED_DATASET_FORMAT = "nnue-v42-lichess-game-disjoint-v2"
CONTROL_CACHE_SCHEMA = "nnue_v43_v36_root_cache_v1"
...
# V43 is a deliberately isolated experiment.  These values are sent to the
# evaluator and then independently checked in its JSON response.  Merely
# relying on the C++ defaults would make a resumed run silently change meaning
# after an unrelated engine edit.
CANDIDATE_ENGINE_PROFILE = {
...
ADAPTIVE_DISABLED_BASELINE = {
    "name": "v43_legacy_fixed50",
    "adaptive_aspiration_enabled": False,
    "legacy_initial_half_window_cp": 50,
}
...
# promoted Fast+QSEE profile instead of inheriting mutable C++ defaults.
...
FROZEN_MAX_RESEARCHES = 6
FROZEN_MEAN_SCORE_CLAMP_CP = 1_500
...
# The three selected V42 profiles are useful global anchors, not promoted V43
# defaults.  They are reevaluated from scratch under V43 scalar/single-bound
# semantics and can be dominated like any other enabled candidate.
PRIOR_V42_ANCHORS = (
    ("fast", 4, 10, 40_000, 2_360, 2, 640),
    ("balanced", 2, 21, 7_300, 2_000, 1, 480),
    ("safe", 5, 65, 12_000, 2_340, 0, 420),
)
...
class Config:
    ...
    @property
    def label(self) -> str:
        prefix = "v43-legacy-fixed50" if not self.enabled else "v43"
...
def candidate_engine_evaluator_args() -> list[str]:
    """Freeze every TT policy that can change the V43 search tree."""
    return [
        "--tt-mb", str(CANDIDATE_ENGINE_PROFILE["tt_mb"]),
        "--tt-bucket-size",
        str(CANDIDATE_ENGINE_PROFILE["tt_bucket_size"]),
...
def parse_dataset_row(line: str, path: Path, line_number: int) -> tuple[str, str, int, str]:
    ...
    if len(parts) != 4:
        raise RuntimeError(
            f"{path}:{line_number}: V43 requires four-column {DATASET_SCHEMA}")
...
def control_cache_spec(
    ...
    identity = {
        ...
        "evaluator_runtime_profile": {
            "candidate_engine": CANDIDATE_ENGINE_PROFILE,
            "selective_config": PRODUCTION_SELECTIVE_CONFIG,
            "twofold_search_draw_enabled": TWOFOLD_SEARCH_DRAW_ENABLED,
            "adaptive_disabled_baseline": ADAPTIVE_DISABLED_BASELINE,
        },
    }
    return {
        "cache_key": cache_key,
...
def ensure_control_cache(
    ...
    if not path.exists():
        ...
        command = [
            ...
            *candidate_engine_evaluator_args(),
            *selective_evaluator_args(),
            *Config(enabled=False).evaluator_args(),
        ]
        try:
...
def validate_sealed_dataset_manifest(
    ...
    if not path.exists():
        ...
        raise RuntimeError(
            f"missing sealed aspiration dataset manifest: {path}; "
    ...
    # Reuse the already sealed, game-disjoint corpus.  V43 changes search
    # semantics, not samples; inventing a new format label for identical bytes
    # would provide no additional isolation.  The full manifest and all TSVs
    # are fingerprinted again in the V43 run manifest below.
    if manifest.get("format") != SEALED_DATASET_FORMAT:
    ...
    for split, dataset in datasets.items():
        ...
        for line_number, line in enumerate(dataset.read_text().splitlines(), 1):
            ...
            if category not in {f"phase{index}" for index in range(8)}:
                raise RuntimeError(
                    f"{dataset}:{line_number}: sealed category must be phase0..7")
            if payload.startswith("book:"):
                raise RuntimeError(
                    f"{dataset}:{line_number}: sealed row must contain FEN")
            try:
            ...
            try:
                ...
                raise RuntimeError(
                    f"{dataset}:{line_number}: invalid sealed FEN") from error
...
def anchors(max_researches: int, mean_score_clamp_cp: int) -> list[Proposal]:
    ...
    seed = Config(**guard)
    result = [
        Proposal(replace(seed, enabled=False), "anchor:v43-legacy-fixed50"),
        Proposal(seed, "anchor:v43-default-seed"),
        Proposal(replace(
    ...
    result.extend(
        Proposal(
            replace(
                seed,
                min_depth=min_depth,
                delta_base_cp=delta_base_cp,
                delta_divisor=delta_divisor,
                expansion_factor_per_mille=expansion,
                max_fail_high_reductions=fail_high_reductions,
                mean_score_new_weight_per_mille=mean_weight,
            ),
            f"anchor:prior-v42-{name}",
        )
        for (
            name, min_depth, delta_base_cp, delta_divisor, expansion,
            fail_high_reductions, mean_weight,
        ) in PRIOR_V42_ANCHORS
    )
    return result
...
def adaptive_frontier(entries: Sequence[dict]) -> list[dict]:
    """Pareto-rank V43 adaptive configs independently from legacy fixed-50.

    The adaptive-off path still uses V43's legacy +/-50 cp aspiration window;
    it is useful for reporting but must not erase every adaptive parent during
    global refinement.  Promotion is checked against it at selection/holdout.
    """
    adaptive = [
...
def validate_result(
    ...
    required = {
        ...
        "candidate_time_ms", "candidate_mean_depth",
        "candidate_completed_depth_count",
        "mean_wdl_loss", "p95_wdl_loss",
    ...
    if result["candidate_search_mode"] != "iterative_depth":
        raise RuntimeError("V43 evaluator bypassed iterative aspiration search")
    if int(result["candidate_time_ms"]) != candidate_time_ms:
        raise RuntimeError("evaluator did not honor candidate-time mode")
    if int(result["candidate_completed_depth_count"]) != count:
        raise RuntimeError("V43 evaluator did not complete every requested depth")
    ...
    for name, expected in CANDIDATE_ENGINE_PROFILE.items():
        if result[name] != expected:
            raise RuntimeError(
                f"evaluator candidate profile {name} differs from frozen "
                f"V43 value {expected!r}")
    if (
    ...
    if (
        ...
        raise RuntimeError(
            "evaluator effective selective config differs from production V43")
    ...
    if int(result["aspiration_range_conflict_fallbacks"]) != 0:
        raise RuntimeError("V43 single-bound aspiration reported a range conflict")
    if int(result["aspiration_unresolved_ranges"]) != 0:
        raise RuntimeError("V43 single-bound aspiration returned unresolved output")
    ...
    if candidate_time_ms == 0:
        if float(result.get("candidate_stopped_pct", 0.0)) != 0.0:
            raise RuntimeError("fixed-depth V43 evaluation stopped early")
        if not math.isclose(
            float(result["candidate_mean_depth"]), float(depth),
            rel_tol=0.0, abs_tol=1e-12,
        ):
            raise RuntimeError("fixed-depth V43 evaluator returned wrong depth")
        accepted_depth_ratio = float(
            result["aspiration_accepted_depth_ratio"])
        if not math.isfinite(accepted_depth_ratio):
            raise RuntimeError("non-finite aspiration accepted-depth ratio")
        if not 0.0 <= accepted_depth_ratio <= 1.0:
            raise RuntimeError("aspiration accepted-depth ratio outside [0, 1]")
        if config.enabled:
            ...
            if not math.isclose(
                accepted_depth_ratio, expected_depth_ratio,
                rel_tol=1e-12, abs_tol=1e-15,
            ):
                raise RuntimeError(
            ...
            if accepted_depth_ratio != 1.0:
                raise RuntimeError(
                    "disabled aspiration must report accepted-depth ratio 1")
            nonzero = {
                name: result[name]
...
def manifest_for(
    ...
    body = {
        "schema_version": SCHEMA_VERSION,
        "experiment": "v43-scalar-single-bound-aspiration-v1",
        "dataset_schema": DATASET_SCHEMA,
        "sealed_dataset_format": SEALED_DATASET_FORMAT,
        ...
        "adaptive_disabled_baseline": ADAPTIVE_DISABLED_BASELINE,
        "candidate_engine_evaluator_args": candidate_engine_evaluator_args(),
        "config_fields": list(CONFIG_FIELDS),
        "telemetry_fields": list(TELEMETRY_FIELDS),
        "derived_telemetry_fields": ["aspiration_accepted_depth_ratio"],
        "completion_fields": [
            "candidate_completed_depth_count", "candidate_mean_depth"],
        "search_bounds": {
            "min_depth": [2, 5],
            "delta_base_cp": [10, 90],
...
def relative_profile(entry: dict, baseline: dict) -> dict:
    payload = serializable_profile([entry])[0]
    result = entry["result"]
    baseline_result = baseline["result"]
    baseline_nodes = float(baseline_result["candidate_nodes"])
    payload["node_ratio_vs_v43_legacy_fixed50"] = (
        float(result["candidate_nodes"]) / baseline_nodes
        if baseline_nodes > 0.0 else math.inf)
    payload["wdl_loss_delta_vs_v43_legacy_fixed50"] = (
        float(result["mean_wdl_loss"])
        - float(baseline_result["mean_wdl_loss"]))
    return payload
...
def selfplay_confirmation_args(
    ...
    candidate_name = f"v43_{profile_label}_{candidate.hash[:10]}"
    control_name = "v43_legacy_fixed50_control"
    ...
    arguments = [
        ...
        "--aspiration-profile",
        gauntlet_aspiration_profile(control_name, baseline),
...
def choose_named_profiles(
    enabled_selection_frontier: Sequence[dict],
    adaptive_on_full_frontier: Sequence[dict],
) -> tuple[tuple[tuple[str, dict], ...], str]:
    """Choose names using selection only, never holdout.

    A baseline-inclusive Pareto survivor is promotion-eligible.  If none
    survives, keep the adaptive-only names as diagnostics so the run remains
    informative, but mark their source explicitly and never promote them.
    """
    if adaptive_on_full_frontier:
        pool = list(adaptive_on_full_frontier)
        source = "selection_baseline_inclusive_frontier"
    elif enabled_selection_frontier:
        pool = list(enabled_selection_frontier)
        source = "selection_adaptive_only_diagnostic"
    else:
        return (), "no_adaptive_selection_profiles"
    fastest = min(
        pool, key=lambda entry: (
    ...
    balanced = knee(pool)
    return (
        ("preselected:fastest", fastest),
        ("preselected:balanced-knee", balanced),
        ("preselected:safest", safest),
    ), source


def annotate_profile_frontiers(
    entry: dict,
    baseline: dict,
    selection_frontier_hashes: set[str],
    holdout_frontier_hashes: set[str],
    selection_source: str,
) -> dict:
    payload = relative_profile(entry, baseline)
    config_hash = config_from_record(entry).hash
    selection_on_full = config_hash in selection_frontier_hashes
    holdout_on_full = config_hash in holdout_frontier_hashes
    payload.update({
        "selection_on_full_frontier": selection_on_full,
        "holdout_on_full_frontier": holdout_on_full,
        "profile_selection_source": selection_source,
    })
    if not selection_on_full:
        payload["offline_profile_status"] = (
            "diagnostic_only_not_on_selection_frontier")
    elif not holdout_on_full:
        payload["offline_profile_status"] = "rejected_by_holdout_frontier"
    else:
        payload["offline_profile_status"] = (
            "selection_and_holdout_frontier_survivor")
    return payload


def summarize_offline_promotion(
    selection_flags: Sequence[bool],
    holdout_flags: Sequence[bool],
) -> str:
    if not selection_flags or not all(selection_flags):
        return "diagnostic_only_no_selection_frontier_promotion"
    if all(holdout_flags):
        return "holdout_confirmed_offline_promotion"
    if any(holdout_flags):
        return "holdout_partially_rejected_offline_promotion"
    return "holdout_rejected_offline_promotion"
...
def parse_args() -> argparse.Namespace:
    ...
    if args.candidate_time_ms != 0:
        parser.error(
            "V43 aspiration tuning requires fixed-depth candidate-time-ms=0")
    if args.candidate_max_depth < 2:
        parser.error("--candidate-max-depth must be at least 2")
    if (
        args.max_researches != FROZEN_MAX_RESEARCHES
        or args.mean_score_clamp_cp != FROZEN_MEAN_SCORE_CLAMP_CP
    ):
        parser.error(
            "V43 frozen guards must be --max-researches 6 and "
...
def main() -> None:
    ...
    run_anchors = anchors(
        args.max_researches, args.mean_score_clamp_cp)
    # Same V43 engine/profile on both sides; only adaptive aspiration differs.
    control_proposals = run_anchors[:2]
    ...
    preselected_profiles, profile_selection_source = choose_named_profiles(
        enabled_selection_frontier, adaptive_on_full_frontier)
    preselected_by_origin = dict(preselected_profiles)
    preselected_fastest = preselected_by_origin.get("preselected:fastest")
    preselected_balanced = preselected_by_origin.get(
        "preselected:balanced-knee")
    preselected_safest = preselected_by_origin.get("preselected:safest")
    ...
    baseline_holdout = holdout_by_hash[control_proposals[0].config.hash]
    ...
    holdout_frontier_hashes = {
        config_from_record(entry).hash for entry in holdout_frontier
    }
    named_holdouts = (
        fastest_holdout, balanced_holdout, safest_holdout)
    named_selection_flags = tuple(
        config_from_record(entry).hash in full_frontier_hashes
        for entry in named_holdouts)
    named_holdout_flags = tuple(
        config_from_record(entry).hash in holdout_frontier_hashes
        for entry in named_holdouts)
    offline_promotion_status = summarize_offline_promotion(
        named_selection_flags, named_holdout_flags)
    fastest_profile = annotate_profile_frontiers(
        fastest_holdout, baseline_holdout, full_frontier_hashes,
        holdout_frontier_hashes, profile_selection_source)
    balanced_profile = annotate_profile_frontiers(
        balanced_holdout, baseline_holdout, full_frontier_hashes,
        holdout_frontier_hashes, profile_selection_source)
    safest_profile = annotate_profile_frontiers(
        safest_holdout, baseline_holdout, full_frontier_hashes,
        holdout_frontier_hashes, profile_selection_source)
    profile_payloads = (fastest_profile, balanced_profile, safest_profile)
    eligible_for_selfplay = tuple(
        bool(profile["selection_on_full_frontier"])
        and bool(profile["holdout_on_full_frontier"])
    ...
    summary = {
        ...
        "adaptive_selection_on_full_frontier_count": len(
            adaptive_on_full_frontier),
        ...
        "adaptive_selection_frontier": serializable_profile(
            enabled_selection_frontier),
        "adaptive_selection_on_full_frontier": serializable_profile(
            adaptive_on_full_frontier),
        "holdout_frontier": serializable_profile(holdout_frontier),
        "fastest": fastest_profile,
        "balanced_knee": balanced_profile,
        "safest": safest_profile,
        "v43_legacy_fixed50_control": relative_profile(
            baseline_holdout, baseline_holdout),
        "v43_seed_control": relative_profile(seed_holdout, baseline_holdout),
        "sealed_candidate_count": len(holdout_proposals),
        "profile_selection_used_holdout": False,
        "enabled_profiles_are_challengers_not_winners": True,
        "requires_equal_time_selfplay": any(eligible_for_selfplay),
        ...
        "equal_time_selfplay_config_args": {
            "fastest": selfplay_confirmation_args(
                "fastest", fastest_holdout, baseline_holdout,
                eligible_for_selfplay[0]),
            "balanced_knee": selfplay_confirmation_args(
                "balanced", balanced_holdout, baseline_holdout,
                eligible_for_selfplay[1]),
            "safest": selfplay_confirmation_args(
                "safest", safest_holdout, baseline_holdout,
                eligible_for_selfplay[2]),
        },
    }
