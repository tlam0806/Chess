...
struct Options {
    ...
    int tt_mb = 64;
    int bucket_size = 4;
    int qsearch_promotion_bonus = chess::HeuristicSearcherV21::MoveOrderingWeights{}.qsearch_promotion_bonus;
    int qsearch_good_capture_bonus = chess::HeuristicSearcherV21::MoveOrderingWeights{}.qsearch_good_capture_bonus;
    int qsearch_bad_capture_bonus = chess::HeuristicSearcherV21::MoveOrderingWeights{}.qsearch_bad_capture_bonus;
    int qsearch_see_weight = chess::HeuristicSearcherV21::MoveOrderingWeights{}.qsearch_see_weight;
    int qsearch_captured_value_weight =
        chess::HeuristicSearcherV21::MoveOrderingWeights{}.qsearch_captured_value_weight;
    bool iterative = true;
...
Options parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--only-v21") {
            options.only_v21 = true;
        } else if (arg == "--help") {
            std::cout
                << "Usage: benchmark_v19_v21_bucket_tt [--input path]\n"
                << "                                     [--samples N]\n"
                << "                                     [--depth D]\n"
                << "                                     [--tt-mb MB]\n"
                << "                                     [--bucket-size N]\n"
...
int main(int argc, char** argv) {
    try {
        ...
        for (std::size_t i = 0; i < samples.size(); ++i) {
            ...
            chess::HeuristicSearcherV21::MoveOrderingWeights weights;
            ...
            chess::HeuristicSearcherV21 v21(
                static_cast<std::size_t>(options.tt_mb),
                static_cast<std::size_t>(options.bucket_size),
                3,
                7,
            ...
            const auto v21_start = std::chrono::steady_clock::now();
            const chess::SearchResult v21_result = search(v21, sample.pos, options.depth, options.iterative);
            const auto v21_end = std::chrono::steady_clock::now();

            const auto v21_us = static_cast<std::uint64_t>(
                std::chrono::duration_cast<std::chrono::microseconds>(v21_end - v21_start).count()
            );
            ...
            add_result(v21_totals, v21_result, v21_us);
            add_tt_stats(v21_tt_totals, v21.tt_stats());
            if (!options.only_v21 && v19_result.score != v21_result.score) {
                ++v19_totals.score_mismatches;
                ++v21_totals.score_mismatches;
            }
            if (!options.only_v21 && v19_result.best_move != v21_result.best_move) {
                ++v19_totals.move_mismatches;
                ++v21_totals.move_mismatches;
            }
            ...
            std::cout
                      << " v21_nodes=" << v21_result.nodes
                      << " v21_us=" << v21_us
                      << " v21_score=" << v21_result.score
                      << " v21_best=" << chess::move_to_string(v21_result.best_move);
            if (!options.only_v21 && v19_result.nodes != 0) {
                std::cout << " v21_vs_v19_nodes="
                          << (static_cast<double>(v21_result.nodes) / static_cast<double>(v19_result.nodes));
            }
            if (!options.only_v21 && v19_us != 0) {
                std::cout << " v21_vs_v19_time="
                          << (static_cast<double>(v21_us) / static_cast<double>(v19_us));
            }
            std::cout << '\n' << std::flush;
        }
        ...
        std::cout << "summary"
                  ...
                  << " v21_nodes=" << v21_totals.nodes
                  << " v21_avg_nodes=" << avg_u64(v21_totals.nodes, v21_totals.searched)
                  << " v21_us=" << v21_totals.time_us
                  << " v21_avg_us=" << avg_u64(v21_totals.time_us, v21_totals.searched);
        if (v19_totals.nodes != 0) {
            std::cout << " v21_vs_v19_nodes="
                      << (static_cast<double>(v21_totals.nodes) / static_cast<double>(v19_totals.nodes));
        }
        if (v19_totals.time_us != 0) {
            std::cout << " v21_vs_v19_time="
                      << (static_cast<double>(v21_totals.time_us) / static_cast<double>(v19_totals.time_us));
        }
        ...
        print_tt_summary("v21", v21_tt_totals);
    } catch (const std::exception& error) {
        std::cerr << "benchmark_v19_v21_bucket_tt: " << error.what() << '\n';
        return 1;
    }
}
