...
if ! git -C "$repo_root" rev-parse --verify HEAD >/dev/null 2>&1; then
  ...
  git -C "$repo_root" add \
    .gitignore CMakeLists.txt README.md pyproject.toml \
    apps cmake deploy docs include python src tests tools
...
if [[ ! -d "$data_dir" || ! -f "$data_manifest" ]]; then
  ...
  cd "$repo_root"
  python3 tools/data/build_robotmoon_unique_corpus.py \
...
cd "$repo_root"
bash tools/train/run_old_score_huber200_500m_runpod_resume.sh
