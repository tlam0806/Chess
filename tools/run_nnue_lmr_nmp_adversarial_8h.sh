...
if [[ ! -f "$SAFETY_BANK_DIR/manifest.json" ]]; then
  .venv/bin/python tools/build_nnue_selective_safety_bank.py \
...
if [[ ! -f "$DATASET_DIR/manifest.txt" ]]; then
  if [[ ! -f "$BALANCED_SOURCE" ]]; then
    .venv/bin/python tools/generate_stockfish_balanced_openings.py \
...
.venv/bin/python tools/tune_nnue_lmr_nmp_adversarial.py \
  --binary build-release/evaluate_nnue_v38_selective \
